...
static NSString *kConst_dateValue = @"UITableViewCell";

//: @interface ZZZCommonTableDelegate()
@interface ColumnShould()
...
@implementation ColumnShould

//: - (instancetype) initWithTableData:(NSArray *(^)(void))data{
- (instancetype) initWithItem:(NSArray *(^)(void))data{
    //: self = [super init];
    self = [super init];
    //: if (self) {
    if (self) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    CellItem *tableSection = self.message[section];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    CellItem *tableSection = self.message[indexPath.section];
    //: NIMCommonTableRow *tableRow = tableSection.rows[indexPath.row];
    TextTap *tableRow = tableSection.rows[indexPath.row];
    //: NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : DefaultTableCell;
    NSString *identity = tableRow.cellClassName.length ? tableRow.cellClassName : kConst_dateValue;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!tableRow.forbidSelect) {
        ...
        UIViewController *vc = tableView.towardSend;
...
- (void)tableView:(UITableView *)tableView willDisplayCell:(UITableViewCell *)cell forRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //: }else{
        ...
        CellItem *section = self.message[indexPath.section];
...
- (void)sessionText:(TextTap *)rowData view:(UITableViewCell *)cell{
