...
@implementation ChromaticView

//: + (instancetype)viewWithBadgeTip:(NSString *)badgeValue{
+ (instancetype)to:(NSString *)badgeValue{
    //: if (!badgeValue) {
    if (!badgeValue) {
        //: badgeValue = @"";
        badgeValue = @"";
    ...
    ChromaticView *instance = [[ChromaticView alloc] init];
    //: instance.frame = [instance frameWithStr:badgeValue];
    instance.frame = [instance spectaclesVideoSmart:badgeValue];
...
- (void)drawRect:(CGRect)rect
{
    ...
    if ([[self badgeValue] length]) {
        ...
        [self aggregationCommentContext:rect session:context];
    //: }else{
    }else{
        //: [self drawWithOutContent:rect context:context];
        [self underItemBy:rect choose:context];
    }
...
- (void)setBadgeValue:(NSString *)badgeValue {
    ...
    self.frame = [self spectaclesVideoSmart:badgeValue];
...
- (CGSize)replacementSuccess:(NSString *)badgeValue{
...
- (CGRect)spectaclesVideoSmart:(NSString *)badgeValue{
    //: CGSize badgeSize = [self badgeSizeWithStr:badgeValue];
    CGSize badgeSize = [self replacementSuccess:badgeValue];
...
- (void)aggregationCommentContext:(CGRect)rect session:(CGContextRef)context{
...
- (void)underItemBy:(CGRect)rect choose:(CGContextRef)context{
    //: CGRect bodyFrame = self.bounds;
    CGRect bodyFrame = self.bounds;
