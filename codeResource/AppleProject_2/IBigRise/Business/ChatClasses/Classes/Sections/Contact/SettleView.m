...
NSString *StringFromNaturalEventData(Byte *data);        


//: contact_tag_fragment_sure
Byte kStr_glanceCruiseCosyData[] = {71, 25, 72, 9, 144, 59, 57, 27, 52, 27, 39, 38, 44, 25, 27, 44, 23, 44, 25, 31, 23, 30, 42, 25, 31, 37, 29, 38, 44, 23, 43, 45, 42, 29, 31};


//: contact_tag_fragment_name
Byte kStr_tartName[] = {84, 25, 17, 11, 70, 131, 9, 8, 180, 141, 65, 82, 94, 93, 99, 80, 82, 99, 78, 99, 80, 86, 78, 85, 97, 80, 86, 92, 84, 93, 99, 78, 93, 80, 92, 84, 14};


//: Required
Byte kStr_voterTitle[] = {49, 8, 82, 12, 29, 129, 159, 1, 101, 198, 100, 123, 0, 19, 31, 35, 23, 32, 19, 18, 249};


//: activity_group_nickname_title
Byte kStr_worryAddData[] = {44, 29, 96, 11, 86, 118, 115, 180, 50, 97, 107, 1, 3, 20, 9, 22, 9, 20, 25, 255, 7, 18, 15, 21, 16, 255, 14, 9, 3, 11, 14, 1, 13, 5, 255, 20, 9, 20, 12, 5, 77};


//: #34AECA
Byte kStr_decreaseName[] = {86, 7, 89, 13, 254, 193, 135, 138, 11, 180, 184, 243, 63, 202, 218, 219, 232, 236, 234, 232, 186};


//: contact_tag_fragment_cancel
Byte kStr_sculptureName[] = {76, 27, 23, 5, 207, 76, 88, 87, 93, 74, 76, 93, 72, 93, 74, 80, 72, 79, 91, 74, 80, 86, 78, 87, 93, 72, 76, 74, 87, 76, 78, 85, 19};


//: #3996D6
Byte kStr_raceName[] = {21, 7, 20, 4, 15, 31, 37, 37, 34, 48, 34, 124};


//: F9F9F9
Byte kStr_pooData[] = {77, 6, 92, 7, 152, 86, 117, 234, 221, 234, 221, 234, 221, 231};


//: 666666
Byte kStr_wheatName[] = {19, 6, 67, 9, 23, 224, 248, 5, 187, 243, 243, 243, 243, 243, 243, 16};
...
@interface SettleView ()<UITextFieldDelegate>
...
- (UILabel *)titleLabel {
    ...
    if (!_titleLabel) {
        ...
        _titleLabel.text = [LabelCell tinkleKey:StringFromNaturalEventData(kStr_worryAddData)];
    }
    //: return _titleLabel;
    return _titleLabel;
}
...
- (UILabel *)subtitleLabel {
    ...
    if (!_subtitleLabel) {
        ...
        _subtitleLabel.textColor = [UIColor box:StringFromNaturalEventData(kStr_decreaseName)];
        //: _subtitleLabel.text = [NTESLanguageManager getTextWithKey:@"Required"];
        _subtitleLabel.text = [LabelCell tinkleKey:StringFromNaturalEventData(kStr_voterTitle)];
    }
...
- (UILabel *)numLabel{
    ...
    if (!_numLabel) {
        ...
        _numLabel.textColor = [UIColor box:StringFromNaturalEventData(kStr_wheatName)];
    }
    //: return _numLabel;
    return _numLabel;
}
...
- (UIButton *)closeBtn {
    ...
    if (!_closeBtn) {
        ...
        [_closeBtn setTitleColor:[UIColor box:StringFromNaturalEventData(kStr_wheatName)] forState:UIControlStateNormal];
        //: [_closeBtn setTitle:[NTESLanguageManager getTextWithKey:@"contact_tag_fragment_cancel"] forState:UIControlStateNormal];
        [_closeBtn setTitle:[LabelCell tinkleKey:StringFromNaturalEventData(kStr_sculptureName)] forState:UIControlStateNormal];
...
- (UIButton *)sureBtn {
    ...
    if (!_sureBtn) {
        ...
        [_sureBtn setTitle:[LabelCell tinkleKey:StringFromNaturalEventData(kStr_glanceCruiseCosyData)] forState:UIControlStateNormal];
        //: _sureBtn.backgroundColor = [UIColor colorWithHexString:@"#34AECA"];
        _sureBtn.backgroundColor = [UIColor box:StringFromNaturalEventData(kStr_decreaseName)];
        //: _sureBtn.layer.cornerRadius = 10;
        _sureBtn.layer.cornerRadius = 10;
        //: _sureBtn.layer.shadowColor = [UIColor colorWithHexString:@"#3996D6"].CGColor;
        _sureBtn.layer.shadowColor = [UIColor box:StringFromNaturalEventData(kStr_raceName)].CGColor;
...
- (UIView *)lineView {
    ...
    if (!_lineView) {
        ...
        _lineView.backgroundColor = [UIColor box:StringFromNaturalEventData(kStr_pooData)];
    }
    //: return _lineView;
    return _lineView;
}
...
- (UIView *)searchView{
    ...
    if(!_searchView){
        ...
        _searchView.backgroundColor = [UIColor box:StringFromNaturalEventData(kStr_pooData)];
        //: _searchView.layer.cornerRadius = 8;
        _searchView.layer.cornerRadius = 8;
        //: _searchView.layer.borderWidth = 1;
        _searchView.layer.borderWidth = 1;
        //: _searchView.layer.borderColor = [UIColor colorWithHexString:@"#34AECA"].CGColor;
        _searchView.layer.borderColor = [UIColor box:StringFromNaturalEventData(kStr_decreaseName)].CGColor;

        //: _searchField = [[UITextField alloc]initWithFrame:CGRectMake(15, 6, [[UIScreen mainScreen] bounds].size.width-80-30, 40)];
        _searchField = [[UITextField alloc]initWithFrame:CGRectMake(15, 6, [[UIScreen mainScreen] bounds].size.width-80-30, 40)];
        //: _searchField.placeholder = [NTESLanguageManager getTextWithKey:@"contact_tag_fragment_name"];
        _searchField.placeholder = [LabelCell tinkleKey:StringFromNaturalEventData(kStr_tartName)];
...
Byte * NaturalEventDataToCache(Byte *data) {
    int distinctPant = data[0];
    int exhibition = data[1];
    Byte voiceMerchant = data[2];
    int hostageAggressive = data[3];
    if (!distinctPant) return data + hostageAggressive;
    for (int i = hostageAggressive; i < hostageAggressive + exhibition; i++) {
        int value = data[i] + voiceMerchant;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[hostageAggressive + exhibition] = 0;
    return data + hostageAggressive;
}

NSString *StringFromNaturalEventData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)NaturalEventDataToCache(data)];
}
