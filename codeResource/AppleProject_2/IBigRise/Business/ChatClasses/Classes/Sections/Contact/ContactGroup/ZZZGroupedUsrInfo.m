...
- (NSString *)sound{
    ...
    NSString *title = [[SimulationCenter centreOff] language:self.max].capitalizedString;
    //: unichar character = [title characterAtIndex:0];
    unichar character = [title characterAtIndex:0];
    //: if (character >= 'A' && character <= 'Z') {
    if (character >= 'A' && character <= 'Z') {
...
- (id)mostValuablePlayerKey{
    ...
    return [[SimulationCenter centreOff] tingUponSnapline:self.max].shortSpelling;
}
