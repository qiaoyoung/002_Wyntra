...
@interface ShareDistance()

//: @property (nonatomic,strong) NIMSession *session;
@property (nonatomic,strong) NIMSession *session;

//: @property (nonatomic,strong) ZZZSessionMsgDatasource *dataSource;
@property (nonatomic,strong) ModelDatasource *dataSource;
                         ...
                         configIndex:(id<PastConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        _dataSource = [[ModelDatasource alloc] initWithProgress:_session withSessionGreen:_sessionConfig];
    }
    //: return self;
    return self;
}
...
- (RegisterResult *)cypher:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource file:models];
...
- (RegisterResult *)can:(NSArray *)models
{
    ...
    NSArray *indexpaths = [self.dataSource tag:models];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    RegisterResult *result = [[RegisterResult alloc] init];
    //: result.indexpaths = indexpaths;
    result.indexpaths = indexpaths;
...
- (RegisterResult *)nameTheoryModel:(CollectionModel *)model
{
    ...
    NSArray *indexs = [self.dataSource background:model];
    //: NIMSessionMessageOperateResult *result = [[NIMSessionMessageOperateResult alloc] init];
    RegisterResult *result = [[RegisterResult alloc] init];
...
- (RegisterResult *)item:(CollectionModel *)model
{
    ...
    NSInteger index = [self.dataSource halogen:model];
...
- (NSInteger)beforeKey:(CollectionModel *)model
{
    ...
    return [self.dataSource halogen:model];
}
...
- (NSArray *)appModels:(NSRange)range
{
    ...
    return [self.dataSource red:range];
}
...
- (CollectionModel *)afterForRed:(NIMMessage *)message{
    ...
    for (CollectionModel *item in self.dataSource.items.reverseObjectEnumerator.allObjects) {
        //: if ([item isKindOfClass:[ZZZMessageModel class]] && [item.message.messageId isEqual:message.messageId]) {
        if ([item isKindOfClass:[CollectionModel class]] && [item.message.messageId isEqual:message.messageId]) {
            //: model = item;
            model = item;
...
- (void)searched:(void(^)(NSError *error, NSArray *))handler
{
    ...
    [self.dataSource enable:handler];
}
...
- (void)changeLength:(void (^)(NSError *))handler
{
    ...
    [self.dataSource bombUpDataRequest:handler];
}
...
- (void)info:(NIMMessage *)message disable:(void (^)(NSError *))handler
{
    ...
    [self.dataSource sub:message list:handler];
}
...
- (void)allowImage:(NIMMessage *)message display:(void (^)(NSError *))handler
{
    ...
    [self.dataSource arrayCallback:message messageUnpackTabImmobilizeUnwanted:handler];
}
        ...
        if ([item isKindOfClass:[CollectionModel class]]) {
            ...
            if (message.isOutgoingMsg) {
                ...
                if (!findLastReceipt) {
                    ...
                    if (message.isRemoteRead && hasConfig && [self.sessionConfig sendDeleteMessage:message])
                    {
...
- (NSDictionary *)jv:(NSArray<NIMMessageReceipt *> *)receipts
{
        ...
        if ([item isKindOfClass:[CollectionModel class]])
        {
            ...
            if (message.isOutgoingMsg)
            {
                ...
                if (message.setting.teamReceiptEnabled &&
                    ...
                    [self.sessionConfig sendDeleteMessage:message])
                {
                    //: model.shouldShowReadLabel = YES;
                    model.shouldShowReadLabel = YES;
                    //: dict[@(i)] = model;
...
- (void)nameView:(NSArray *)messages
{
    ...
    for (NSInteger i = [messages count] - 1; i >= 0; i--) {
        ...
        else if ([item isKindOfClass:[CollectionModel class]])
        {
            //: message = [(ZZZMessageModel *)item message];
            message = [(CollectionModel *)item message];
        }
        ...
        if (message)
        {
            ...
            if (!message.isOutgoingMsg &&
                ...
                [self.sessionConfig respondsToSelector:@selector(sendDeleteMessage:)] &&
                //: [self.sessionConfig shouldHandleReceiptForMessage:message])
                [self.sessionConfig sendDeleteMessage:message])
            {
...
- (void)grievousBodilyHarm:(NSArray *)messages
{
