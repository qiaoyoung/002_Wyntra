...
NSString *StringFromBlanketData(Byte *data);


//: icon_file
Byte kStr_identityName[] = {50, 9, 3, 101, 108, 105, 102, 95, 110, 111, 99, 105, 228};

// __DEBUG__
// __CLOSE_PRINT__
//
//  VisibleLightTitleView.m
//  NIM
//
...
#import "VisibleLightTitleView.h"
...
@interface VisibleLightTitleView()
...
@implementation VisibleLightTitleView
...
- (instancetype)initDistance{
    ...
    if (self) {
        ...
        UIImage * image = [UIImage imageNamed:StringFromBlanketData(kStr_identityName)];
...
- (void)text:(CollectionModel *)data{
    ...
    self.titleLabel.font = [[On along].config style:data.message].font;
    ...
    self.sizeLabel.font = [[On along].config style:data.message].font;
...
Byte * BlanketDataToCache(Byte *data) {
    int finance = data[0];
    int shrinkEntertain = data[1];
    int lysisHouse = data[2];
    if (!finance) return data + lysisHouse;
    for (int i = 0; i < shrinkEntertain / 2; i++) {
        int begin = lysisHouse + i;
        int end = lysisHouse + shrinkEntertain - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[lysisHouse + shrinkEntertain] = 0;
    return data + lysisHouse;
}

NSString *StringFromBlanketData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)BlanketDataToCache(data)];
}  
