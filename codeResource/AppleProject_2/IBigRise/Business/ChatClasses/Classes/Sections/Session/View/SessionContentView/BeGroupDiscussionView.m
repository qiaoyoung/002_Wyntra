...
NSString *StringFromMysteryBillData(Byte *data);


//: status
Byte kStr_technologyMessageRapidText[] = {86, 6, 37, 11, 5, 186, 95, 205, 201, 54, 47, 152, 153, 134, 153, 154, 152, 108};

// __DEBUG__
// __CLOSE_PRINT__
//
//  BeGroupDiscussionView.m
// On
//
...
#import "BeGroupDiscussionView.h"
...
@interface BeGroupDiscussionView()
...
@property (nonatomic,strong) AlongView * progressView;
...
@implementation BeGroupDiscussionView
...
//        self.progress.tintColor = [UIColor colorWithPatternImage:[VisualImageDisable getLinearGradientImage:RGB_COLOR_String(kCommonBGColor_begin) and:RGB_COLOR_String(kCommonBGColor_end) directionType:SNLinearGradientDirectionLevel]];
...
//        [self addSubview:_playBtn];
        ...
        _progressView = [[AlongView alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context{
    ...
    if ([keyPath isEqualToString:StringFromMysteryBillData(kStr_technologyMessageRapidText)]) {
...
- (void)text:(CollectionModel *)data{
    ...
    [_playerItem removeObserver:self forKeyPath:StringFromMysteryBillData(kStr_technologyMessageRapidText) context:kConst_memberName];
    ...
    [_playerItem addObserver:self forKeyPath:StringFromMysteryBillData(kStr_technologyMessageRapidText) options:NSKeyValueObservingOptionNew context:kConst_memberName];
...
//    _progressView.hidden         = YES;
    ...
    if (!_progressView.hidden) {
        ...
        [_progressView setSpeedy:[[[NIMSDK sharedSDK] chatManager] messageTransportProgress:self.model.message]];
    }
}
...
- (ChangeController *)avPlayer {
    ...
    if (!_avPlayer) {
        ...
        _avPlayer = [[ChangeController alloc] initWithShow:self.fileURL];
...
- (void)scale:(float)progress
{
    ...
    self.progressView.speedy = progress;
}


//: - (UIImage *)thumbnailImageForVideo:(NSURL *)videoURL atTime:(NSTimeInterval)time
- (UIImage *)getRolling:(NSURL *)videoURL image:(NSTimeInterval)time
{
    ...
    thumbnailImage = [thumbnailImage pastSize:CGSizeMake(600, 600)];
...
Byte * MysteryBillDataToCache(Byte *data) {
    int illusion = data[0];
    int concernedIndex = data[1];
    Byte fastenerMessage = data[2];
    int importance = data[3];
    if (!illusion) return data + importance;
    for (int i = importance; i < importance + concernedIndex; i++) {
        int value = data[i] - fastenerMessage;
        if (value < 0) {
            value += 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[importance + concernedIndex] = 0;
    return data + importance;
}

NSString *StringFromMysteryBillData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)MysteryBillDataToCache(data)];
}
