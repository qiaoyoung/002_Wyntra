...
Byte kStr_hardText[] = {27, 21, 4, 16, 100, 108, 37, 100, 108, 37, 108, 108, 101, 67, 110, 111, 116, 116, 117, 66, 101, 108, 98, 97, 116, 23};


//: common_bg
Byte kStr_companyValue[] = {91, 9, 7, 1, 104, 254, 98, 103, 98, 95, 110, 111, 109, 109, 111, 99, 6};


//: contact_tag_fragment_cancel
Byte kStr_courtroomData[] = {54, 27, 3, 108, 101, 99, 110, 97, 99, 95, 116, 110, 101, 109, 103, 97, 114, 102, 95, 103, 97, 116, 95, 116, 99, 97, 116, 110, 111, 99, 124};


//: back_arrow_bl
Byte kStr_charmData[] = {17, 13, 10, 101, 142, 115, 82, 235, 127, 109, 108, 98, 95, 119, 111, 114, 114, 97, 95, 107, 99, 97, 98, 29};


//: TableSwitch%ld%ld
Byte kStr_importanceValue[] = {47, 17, 11, 53, 219, 206, 103, 194, 203, 113, 82, 100, 108, 37, 100, 108, 37, 104, 99, 116, 105, 119, 83, 101, 108, 98, 97, 84, 218};


//: tableButtonCell
Byte kStr_muscleActuallyName[] = {40, 15, 13, 76, 106, 62, 46, 5, 169, 40, 46, 129, 71, 108, 108, 101, 67, 110, 111, 116, 116, 117, 66, 101, 108, 98, 97, 116, 119};


//: builidCommonCell%ld%ld
Byte kStr_theoreticalOofData[] = {82, 22, 11, 220, 30, 149, 151, 29, 185, 35, 197, 100, 108, 37, 100, 108, 37, 108, 108, 101, 67, 110, 111, 109, 109, 111, 67, 100, 105, 108, 105, 117, 98, 70};


//: 未设置
Byte kStr_himMysteryValue[] = {56, 9, 6, 236, 178, 164, 174, 189, 231, 190, 174, 232, 170, 156, 230, 137};


//: tableMemberCell
Byte kStr_forestMinderData[] = {28, 15, 10, 161, 9, 97, 201, 172, 152, 241, 108, 108, 101, 67, 114, 101, 98, 109, 101, 77, 101, 108, 98, 97, 116, 202};
...
@interface SectionViewController () <UITableViewDataSource,
                                         //: UITableViewDelegate,
                                         UITableViewDelegate,
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"common_bg"]];
    ...
    bg.image = [UIImage imageNamed:StringFromRuddyData(kStr_companyValue)];
...
//    [self.view addSubview:bgimg];
    ...
    backButton.frame = CGRectMake(5, [UIDevice encounter], 40, 40);
    //: [backButton setImage:[UIImage imageNamed:@"back_arrow_bl"] forState:(UIControlStateNormal)];
    [backButton setImage:[UIImage imageNamed:StringFromRuddyData(kStr_charmData)] forState:(UIControlStateNormal)];
...
- (UIAlertAction *)iconName {
    ...
    UIAlertAction *cancel = [UIAlertAction actionWithTitle:[LabelCell tinkleKey:StringFromRuddyData(kStr_courtroomData)]
...
- (id<NTESCardBodyData>)sub:(NSIndexPath*)indexpath{
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    id<NTESCardBodyData> bodyData = [self sub:indexPath];
...
- (UITableViewCell*)progress:(id<NTESCardBodyData>) bodyData success:(NSIndexPath *)indexPath
{
    //: NSString *identifier = [NSString stringWithFormat:@"builidCommonCell%ld%ld",(long)indexPath.section,(long)indexPath.row];
    NSString *identifier = [NSString stringWithFormat:StringFromRuddyData(kStr_theoreticalOofData),(long)indexPath.section,(long)indexPath.row];
    //: ZMONGroupEditTableViewCell * cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    QuantityervalViewCell * cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    //: if (!cell) {
    if (!cell) {
        //: cell = [[ZMONGroupEditTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        cell = [[QuantityervalViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    ...
    if ([bodyData respondsToSelector:@selector(subTitle)]) {
        ...
        cell.contentLabel.text = bodyData.subTitle ?: [LabelCell tinkleKey:StringFromRuddyData(kStr_himMysteryValue)];
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (bodyData.type == TeamCardRowItemTypeSelected) {
        ...
        InputViewController *vc = [InputViewController size:bodyData.title
...
- (UITableViewCell*)clear:(id<NTESCardBodyData>) bodyData mode:(NSIndexPath *)indexPath{
//    tableButtonCell
    //: NSString *identifier = [NSString stringWithFormat:@"tableButtonCell%ld%ld",(long)indexPath.section,(long)indexPath.row];
    NSString *identifier = [NSString stringWithFormat:StringFromRuddyData(kStr_hardText),(long)indexPath.section,(long)indexPath.row];
    //: ZZZKitColorButtonCell * cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    WithViewCell * cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    //: if (!cell) {
    if (!cell) {
        //: cell = [[ZZZKitColorButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        cell = [[WithViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
    }
    //: cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    //: cell.button.style = ZZZKitColorButtonCellStyleRed;
    cell.button.style = ZZZKitColorButtonCellStyleRed;
...
- (UITableViewCell*)question:(id<NTESCardBodyData>) bodyData{
    //: ZZZKitColorButtonCell * cell = [self.tableView dequeueReusableCellWithIdentifier:@"tableButtonCell"];
    WithViewCell * cell = [self.tableView dequeueReusableCellWithIdentifier:StringFromRuddyData(kStr_muscleActuallyName)];
    //: if (!cell) {
    if (!cell) {
        //: cell = [[ZZZKitColorButtonCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"tableButtonCell"];
        cell = [[WithViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:StringFromRuddyData(kStr_muscleActuallyName)];
    }
    //: cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    //: cell.button.style = ZZZKitColorButtonCellStyleBlue;
    cell.button.style = ZZZKitColorButtonCellStyleBlue;
    //: [cell.button setTitle:bodyData.title forState:UIControlStateNormal];
    [cell.button setTitle:bodyData.title forState:UIControlStateNormal];
    //: return cell;
...
- (UITableViewCell*)shouldText:(id<NTESCardBodyData>) bodyData{
    //: ZZZTeamMemberListCell * cell = [self.tableView dequeueReusableCellWithIdentifier:@"tableMemberCell"];
    PinSureView * cell = [self.tableView dequeueReusableCellWithIdentifier:StringFromRuddyData(kStr_forestMinderData)];
    //: if (!cell) {
    if (!cell) {
        //: cell = [[ZZZTeamMemberListCell alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:@"tableMemberCell"];
        cell = [[PinSureView alloc] initWithStyle:UITableViewCellStyleValue1 reuseIdentifier:StringFromRuddyData(kStr_forestMinderData)];
    }
    //: cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
...
- (UITableViewCell *)of:(id<NTESCardBodyData>)bodyData readToday:(NSIndexPath *)indexPath
{
    //: NSString *identifier = [NSString stringWithFormat:@"TableSwitch%ld%ld",(long)indexPath.section,(long)indexPath.row];
    NSString *identifier = [NSString stringWithFormat:StringFromRuddyData(kStr_importanceValue),(long)indexPath.section,(long)indexPath.row];
    //: ZZZTeamSwitchTableViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    WithItemViewCell *cell = [self.tableView dequeueReusableCellWithIdentifier:identifier];
    //: cell.selectionStyle = UITableViewCellSelectionStyleNone;
    cell.selectionStyle = UITableViewCellSelectionStyleNone;
    //: if (!cell) {
    if (!cell) {
        //: cell = [[ZZZTeamSwitchTableViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        cell = [[WithItemViewCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:identifier];
        //: CGFloat left = 15.f;
        CGFloat left = 15.f;
        //: CGFloat height = 1.f;
        CGFloat height = 1.f;
...
- (void)setDatas:(NSArray<NSArray<ElevenPath *> *> *)datas {
    //: _datas = datas;
    _datas = datas;
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(15, (44.0f + [UIDevice encounter]), [[UIScreen mainScreen] bounds].size.width-30, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice encounter])) style:UITableViewStyleGrouped];
...
- (UIView *)disableView { return [UIView new]; }
...
Byte * RuddyDataToCache(Byte *data) {
    int bloodRed = data[0];
    int tartComplete = data[1];
    int dominantCommittee = data[2];
    if (!bloodRed) return data + dominantCommittee;
    for (int i = 0; i < tartComplete / 2; i++) {
        int begin = dominantCommittee + i;
        int end = dominantCommittee + tartComplete - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[dominantCommittee + tartComplete] = 0;
    return data + dominantCommittee;
}

NSString *StringFromRuddyData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)RuddyDataToCache(data)];
}  
