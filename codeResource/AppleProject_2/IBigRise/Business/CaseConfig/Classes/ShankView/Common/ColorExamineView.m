...
@implementation ColorExamineView
...
- (NSInteger)fundamentalSum:(NSInteger)value highStatus:(NSInteger)min duringMin:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)filenameMode:(NSInteger)currentPage
{
    ...
    NSInteger first = [self fundamentalSum:currentPage - 1 highStatus:0 duringMin:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self fundamentalSum:currentPage + 1 highStatus:0 duringMin:count - 1];
...
- (void)beat
{
    ...
    [self filenameMode:_currentPage];
