...
NSString *StringFromDriverData(Byte *data);        


//: #3996D6
Byte kStr_novelName[] = {87, 7, 48, 12, 151, 2, 209, 185, 118, 88, 164, 14, 243, 3, 9, 9, 6, 20, 6, 213};


//: contact_list_activity_complete
Byte kStr_userClassicContent[] = {46, 30, 8, 7, 216, 107, 151, 91, 103, 102, 108, 89, 91, 108, 87, 100, 97, 107, 108, 87, 89, 91, 108, 97, 110, 97, 108, 113, 87, 91, 103, 101, 104, 100, 93, 108, 93, 225};


//: #34AECA
Byte kStr_widelyText[] = {11, 7, 69, 14, 186, 19, 5, 18, 104, 95, 214, 5, 25, 229, 222, 238, 239, 252, 0, 254, 252, 224};


//: back_arrow_bl
Byte kStr_studentName[] = {69, 13, 95, 7, 120, 134, 50, 3, 2, 4, 12, 0, 2, 19, 19, 16, 24, 0, 3, 13, 208};


//: common_bg
Byte kStr_galUnlessName[] = {2, 9, 25, 11, 99, 146, 233, 5, 15, 213, 60, 74, 86, 84, 84, 86, 85, 70, 73, 78, 179};


//: ZMONTeamCartSet%ld
Byte kStr_stretchData[] = {90, 18, 1, 6, 56, 214, 89, 76, 78, 77, 83, 100, 96, 108, 66, 96, 113, 115, 82, 100, 115, 36, 107, 99, 102};
...
+ (instancetype)size:(NSString *)title
    ...
    InputViewController *vc = [[InputViewController alloc] initWithDateQuantity:items];
...
- (instancetype)initWithDateQuantity:(NSMutableArray <id <NIMKitSelectCardData>> *)items {
    //: if (self = [super init]) {
    if (self = [super init]) {
        //: _datas = items;
        _datas = items;
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"common_bg"]];
    ...
    bg.image = [UIImage imageNamed:StringFromDriverData(kStr_galUnlessName)];
    ...
    [backButton setImage:[UIImage imageNamed:StringFromDriverData(kStr_studentName)] forState:(UIControlStateNormal)];
...
//    ValueView *cell = [ValueView cellWithTableView:tableView];
    //: NSString *identifier = [NSString stringWithFormat:@"ZMONTeamCartSet%ld",(long)indexPath.row];
    NSString *identifier = [NSString stringWithFormat:StringFromDriverData(kStr_stretchData),(long)indexPath.row];
...
- (UITableView *)tableView {
    ...
    if (!_tableView) {
        ...
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(15, (44.0f + [UIDevice encounter]), [[UIScreen mainScreen] bounds].size.width-30, [[UIScreen mainScreen] bounds].size.height-(44.0f + [UIDevice encounter])) style:UITableViewStylePlain];
...
//        btnClear.layer.cornerRadius = 24;
        ...
        [btnClear setTitle:[LabelCell tinkleKey:StringFromDriverData(kStr_userClassicContent)] forState:UIControlStateNormal];
        //: [btnClear addTarget:self action:@selector(onDone) forControlEvents:UIControlEventTouchUpInside];
        [btnClear addTarget:self action:@selector(alongWithDone) forControlEvents:UIControlEventTouchUpInside];
        //: [_footView addSubview:btnClear];
        [_footView addSubview:btnClear];
        //: btnClear.backgroundColor = [UIColor colorWithHexString:@"#34AECA"];
        btnClear.backgroundColor = [UIColor box:StringFromDriverData(kStr_widelyText)];
        //: btnClear.layer.cornerRadius = 10;
        btnClear.layer.cornerRadius = 10;
        //: btnClear.layer.shadowColor = [UIColor colorWithHexString:@"#3996D6"].CGColor;
        btnClear.layer.shadowColor = [UIColor box:StringFromDriverData(kStr_novelName)].CGColor;
...
Byte * DriverDataToCache(Byte *data) {
    int hostageName = data[0];
    int lemonlike = data[1];
    Byte clinch = data[2];
    int unless = data[3];
    if (!hostageName) return data + unless;
    for (int i = unless; i < unless + lemonlike; i++) {
        int value = data[i] + clinch;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[unless + lemonlike] = 0;
    return data + unless;
}

NSString *StringFromDriverData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)DriverDataToCache(data)];
}
