...
@interface MarginInputItem ()

//: @end
@end

//: @implementation NIMCardMemberItem
@implementation MarginInputItem
...
- (UIImage *)imageNormal{
    ...
    MagnitudeCommentInfo *info = [[On along] toKey:self.userId image:nil];
...
- (NSString *)title {
    ...
    return [TingText text:self.userId keyBegin:session];
}

//: - (NSString *)imageUrl{
- (NSString *)imageUrl{
    //: return [[AppleProjectKit sharedKit] infoByUser:self.userId option:nil].avatarUrlString;
    return [[On along] toKey:self.userId image:nil].avatarUrlString;
}
...
@implementation WithQuantityerval
...
- (BOOL)isEqual:(id)object{
    ...
    if (![object isKindOfClass:[WithQuantityerval class]]) {
        //: return NO;
        return NO;
    }
    //: ZZZTeamCardMemberItem *obj = (ZZZTeamCardMemberItem*)object;
    WithQuantityerval *obj = (WithQuantityerval*)object;
