                         ...
                         modify:(id<PastConfig>)sessionConfig
{
    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(physicalProperty:) name:UIMenuControllerDidHideMenuNotification object:nil];
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(lineWithFrame:) name:kConst_bottomKitName object:nil];
    }
    //: return self;
    return self;
}
...
//    self.tableView.frame = CGRectMake(0, SCREEN_TOP_HEIGHT, SCREEN_WIDTH, SCREEN_HEIGHT);
    ...
    rect.origin.y = rect.origin.y > 0? (44.0f + [UIDevice encounter]) : rect.origin.y;
...
//        rect.size.height -= SCREEN_TOP_HEIGHT;
        ...
        [self.tableView setFrame:rect];
        //: [self.tableView nim_scrollToBottom:YES];
        [self.tableView chromaticColor:YES];
    }
//    [self.tableView nim_scrollToBottom:YES];
}
...
- (void)surveyContent:(CollectionModel *)model{
    ...
    if ([NSThread isMainThread]) {^{ [model info:self.tableView.nim_width]; }();} else {dispatch_sync(dispatch_get_main_queue(), ^{ [model info:self.tableView.nim_width]; });};
...
- (void)read
{
    ...
    [self.refreshControl addTarget:self action:@selector(red:) forControlEvents:UIControlEventValueChanged];
...
- (void)red:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(enableLight)])
    {
        //: [self.delegate onRefresh];
        [self.delegate enableLight];
    }
}

//: - (void)insert:(NSArray<NSIndexPath *> *)indexPaths animated:(BOOL)animated
- (void)data:(NSArray<NSIndexPath *> *)indexPaths selected:(BOOL)animated
{
    //: if (!indexPaths.count)
    if (!indexPaths.count)
    {
...
- (void)names:(NSArray<NSIndexPath *> *)indexPaths
{
    //: if ([self shouldReloadWhenRemoveOrUpdate:indexPaths])
    if ([self center:indexPaths])
    {
        //: [self.tableView reloadData];
        [self.tableView reloadData];
        //: return;
...
- (void)whenFlip:(NSIndexPath *)indexPath
{
    ...
    CypherMediaCompartmentViewCell *cell = (CypherMediaCompartmentViewCell *)[self.tableView cellForRowAtIndexPath:indexPath];
    //: if (cell) {
    if (cell) {
        //: @try {
        @try {
...
- (void)region {
    ...
    if (!self.inputView.replyedContent.hidden)
    {
        ...
        [self.inputView.replyedContent pageDate];
    }
}
...
- (BOOL)center:(NSArray<NSIndexPath *> *)indexPaths
{
    //: for(NSIndexPath * indexPath in indexPaths)
    for(NSIndexPath * indexPath in indexPaths)
    {
