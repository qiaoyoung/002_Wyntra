...
NSString *StringFromGroveData(GroveData *data);


//: speaker
GroveData kStr_withVasPhysicallyName = (GroveData){169, (Byte []){218, 217, 204, 200, 194, 204, 219, 113}, 7, false, 7, 13, 39};


//: lead_close
GroveData kStr_damnConfusionTitle = (GroveData){245, (Byte []){153, 144, 148, 145, 170, 150, 153, 154, 134, 144, 126}, 10, false, 24, 251, 130};


//: click
GroveData kStr_conferenceMessageRangeData = (GroveData){49, (Byte []){82, 93, 88, 82, 90, 93}, 5, false, 242, 148, 246};
...
+ (instancetype)with:(float)delay
    ...
    FogView *tipView = [[FogView alloc] initWithFrame:(CGRect) {
        //: .origin.x = 5,
        .origin.x = 5,
        //: .origin.y = (44.0f + [UIDevice vg_statusBarHeight])+5,
        .origin.y = (44.0f + [UIDevice encounter])+5,
    ...
    [tipView say:^(FogView *tipView) {
        ...
        switch (type) {
            ...
            case CompletingUserInfoType_headicon:
                ...
                tipView.actionTitle = StringFromGroveData(&kStr_conferenceMessageRangeData);
                //: break;
                break;
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.userInteractionEnabled = YES;
        {
            ...
            self.leftwardMarqueeView = [[TapShareView alloc] initWithTillEdit:CGRectMake(54, 0, [UIScreen mainScreen].bounds.size.width - 54 - 30 - 10, 46) paperFeed:UUMarqueeViewDirectionLeftward];
...
//            button.backgroundColor = [UIColor redColor];
            ...
            [button addTarget:self action:@selector(alonging:) forControlEvents:UIControlEventTouchUpInside];
//            [button.titleLabel setFont:[UIFont systemFontOfSize:14]];
...

        {
            ...
            noticeImageview.image = [UIImage imageNamed:StringFromGroveData(&kStr_withVasPhysicallyName)];
...
- (void)say:(void (^)(FogView *tipView))transactionBlock
{
    ...
    [_actionButton setImage:[UIImage imageNamed:StringFromGroveData(&kStr_damnConfusionTitle)] forState:(UIControlStateNormal)];
    ...
    [_leftwardMarqueeView refresh];
    //: [_leftwardMarqueeView start];
    [_leftwardMarqueeView unpack];
}
...
- (void)graphic:(BOOL)callback
{
    //: [self.leftwardMarqueeView pause];
    [self.leftwardMarqueeView displayName];
...
- (NSUInteger)disables:(TapShareView*)marqueeView {
    //: return 1;
    return 1;
}
...
- (CGFloat)trademark:(NSUInteger)index recentTingLength:(TapShareView*)marqueeView {
...
- (CGFloat)marquee:(NSUInteger)index table:(TapShareView*)marqueeView {
    // for leftwardMarqueeView
    //: UILabel *content = [[UILabel alloc] init];
    UILabel *content = [[UILabel alloc] init];
    //: content.font = [UIFont systemFontOfSize:13.0f];
...
Byte *GroveDataToByte(GroveData *data) {
    if (data->receiverDented) return data->vulturine;
    for (int i = 0; i < data->mannerTee; i++) {
        data->vulturine[i] ^= data->galaxy;
    }
    data->vulturine[data->mannerTee] = 0;
    data->receiverDented = true;
	if (data->mannerTee >= 3) {
		data->folkPolicy = data->vulturine[0];
		data->vehicleRout = data->vulturine[1];
		data->national = data->vulturine[2];
	}
    return data->vulturine;
}

NSString *StringFromGroveData(GroveData *data) {
    return [NSString stringWithUTF8String:(char *)GroveDataToByte(data)];
}
