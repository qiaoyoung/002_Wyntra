...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        CopyTextView *textView = [[CopyTextView alloc] initWithFrame:rect];
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder
{
    ...
    if (self) {
        ...
        CopyTextView *textView = [[CopyTextView alloc] initWithFrame:CGRectZero];
        //: self.textView = textView;
        self.textView = textView;
...
- (CGSize)intrinsicContentSize
{
    ...
    return [self glassesBy:self.refer].size;
}
...
- (void)setMinNumberOfLines:(NSInteger)minNumberOfLines
{
    ...
    self.minHeight = [self disableHeight:minNumberOfLines];
...
- (void)setMaxNumberOfLines:(NSInteger)maxNumberOfLines
{
    ...
    self.maxHeight = [self disableHeight:maxNumberOfLines];
...
- (CGFloat)disableHeight:(NSInteger)line
{
    ...
    CGFloat height = self.refer.height - (textViewMargin + self.textView.contentInset.top + self.textView.contentInset.bottom);
...
- (void)face{
    ...
    if (newScrollViewFrame.size.height <= self.maxHeight && _previousTextHeight == 0) {
        ...
        if(oldScrollViewFrame.size.height != newScrollViewFrame.size.height) {
            ...
            if ([self.textViewDelegate respondsToSelector:@selector(infoHeight:)]) {
                //: [self.textViewDelegate willChangeHeight:newScrollViewFrame.size.height];
                [self.textViewDelegate infoHeight:newScrollViewFrame.size.height];
            }
    ...
    if (oldScrollViewFrame.size.height != newScrollViewFrame.size.height && [self.textViewDelegate respondsToSelector:@selector(commutatived:)]) {
        //: [self.textViewDelegate didChangeHeight:newScrollViewFrame.size.height];
        [self.textViewDelegate commutatived:newScrollViewFrame.size.height];
    }
...
- (CGRect)glassesBy:(CGSize)contentSize
{
    //: CGSize selfSize;
    CGSize selfSize;
...
- (BOOL)textView:(UITextView *)textView shouldChangeTextInRange:(NSRange)range replacementText:(NSString *)text
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(indicator:window:)]) {
        //: return [self.textViewDelegate shouldChangeTextInRange:range replacementText:text];
        return [self.textViewDelegate indicator:range window:text];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithURL:(NSURL *)URL inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(ting:should:)]) {
        //: return [self.textViewDelegate shouldInteractWithURL:URL inRange:characterRange];
        return [self.textViewDelegate ting:URL should:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textView:(UITextView *)textView shouldInteractWithTextAttachment:(NSTextAttachment *)textAttachment inRange:(NSRange)characterRange
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(key:should:)]) {
        //: return [self.textViewDelegate shouldInteractWithTextAttachment:textAttachment inRange:characterRange];
        return [self.textViewDelegate key:textAttachment should:characterRange];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(upwardlied:)]) {
        //: [self.textViewDelegate textViewDidBeginEditing:self];
        [self.textViewDelegate upwardlied:self];
    }
}
...
- (void)textViewDidChangeSelection:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(haphazards:)]) {
        //: [self.textViewDelegate textViewDidChangeSelection:self];
        [self.textViewDelegate haphazards:self];
    }
}
...
- (void)textViewDidEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(names:)]) {
        //: [self.textViewDelegate textViewDidEndEditing:self];
        [self.textViewDelegate names:self];
    }
}
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(bared:)]) {
       //: return [self.textViewDelegate textViewShouldBeginEditing:self];
       return [self.textViewDelegate bared:self];
    }
    //: return YES;
    return YES;
}
...
- (BOOL)textViewShouldEndEditing:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(imageEditing:)]) {
        //: return [self.textViewDelegate textViewShouldEndEditing:self];
        return [self.textViewDelegate imageEditing:self];
    }
    //: return YES;
    return YES;
}
...
- (void)textViewDidChange:(UITextView *)textView
{
    ...
    if ([self.textViewDelegate respondsToSelector:@selector(cuts:)]) {
        //: [self.textViewDelegate textViewDidChange:self];
        [self.textViewDelegate cuts:self];
    }
    //: [self fitToScrollView];
    [self face];
}
...
@implementation ByView(TextView)
