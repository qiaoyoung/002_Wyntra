...
@implementation UIImage (Item)
//: + (UIImage *)fetchImage:(NSString *)imageNameOrPath{
+ (UIImage *)bubbleKey:(NSString *)imageNameOrPath{
...
- (UIImage *)direction
{
    ...
    CGFloat pixels = [[CenterValue displayEnable] downGroundImage];
    //: UIImage * image = [self imageForUpload:pixels];
    UIImage * image = [self worldView:pixels];
    //: return [image fixOrientation];
    return [image replyInfo];
}
...
- (UIImage *)worldView: (CGFloat)suggestPixels
{
        ...
        (width / height > kMaxRatio || height / width > kMaxRatio))
    {
        ...
        return [self query:kMaxPixels];
    }
    //: else
    else
    {
        //: return [self scaleWithMaxPixels:suggestPixels];
        return [self query:suggestPixels];
    }
}

//: - (UIImage *)scaleWithMaxPixels: (CGFloat)maxPixels
- (UIImage *)query: (CGFloat)maxPixels
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self store:CGSizeMake(newSizeWidth, newSizeHeight)];
}

//内缩放，一条变等于最长边，另外一条小于等于最长边
//: - (UIImage *)scaleToSize:(CGSize)newSize
- (UIImage *)store:(CGSize)newSize
{
    //: CGFloat width = self.size.width;
    CGFloat width = self.size.width;
    //: CGFloat height= self.size.height;
    ...
    return [self frameAcross:size];
}

//: - (UIImage *)drawImageWithSize: (CGSize)size
- (UIImage *)frameAcross: (CGSize)size
{
