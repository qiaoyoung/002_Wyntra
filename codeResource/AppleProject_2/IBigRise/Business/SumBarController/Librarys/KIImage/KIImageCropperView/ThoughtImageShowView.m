...
@interface ThoughtImageShowView() {
...
@implementation ThoughtImageShowView
...
- (BoardView *)maskView {
    ...
    if (_maskView == nil) {
        ...
        _maskView = [[BoardView alloc] initWithFrame:self.bounds];
...
- (UIImageView *)index {
    ...
    if (_imageView == nil) {
        ...
        [[self input] addSubview:_imageView];
    }
    //: return _imageView;
    return _imageView;
}
...
- (void)produce:(KIImageCropRatio)ratio {
...
- (void)setImage:(UIImage *)image {
    ...
    if (_image != image) {
        ...
        [[self index] setImage:_image];

        //: [[self imageView] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
        [[self index] setFrame:CGRectMake(0, 0, _image.size.width, _image.size.height)];
...
- (void)cellScale {
    ...
    [[self input] setMinimumZoomScale:min];
    //: [[self scrollView] setMaximumZoomScale:max+5.0f];
    [[self input] setMaximumZoomScale:max+5.0f];

    //: [[self scrollView] setZoomScale:min animated:YES];
    [[self input] setZoomScale:min animated:YES];

    //: [[self scrollView] setContentInset:_imageViewInsets];
    [[self input] setContentInset:_imageViewInsets];
    //: [[self scrollView] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
    [[self input] setContentOffset:CGPointMake(-_imageViewInsets.left, -_imageViewInsets.top) animated:YES];
}
...
- (void)layoutSubviews {
    ...
    [(BoardView*)[self maskView] setItem:[self noBlueLink]];
    //: [[self scrollView] setFrame:self.bounds];
    [[self input] setFrame:self.bounds];
    //: [self updateZoomScale];
    [self cellScale];
}

//: - (UIImage *)croppedImage {
- (UIImage *)thoughtImageWith {
    //: CGFloat zoomScale = [self scrollView].zoomScale;
    CGFloat zoomScale = [self input].zoomScale;

    //: CGFloat offsetX = [self scrollView].contentOffset.x;
    CGFloat offsetX = [self input].contentOffset.x;
    //: CGFloat offsetY = [self scrollView].contentOffset.y;
    CGFloat offsetY = [self input].contentOffset.y;
    ...
    image = [self nomographWithoutPin:image with:imgSize withCenter:YES];
...
- (UIImage*)nomographWithoutPin:(UIImage*)image with:(CGSize)newSize withCenter:(BOOL)opaque
{
