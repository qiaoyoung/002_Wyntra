...
@interface CommentScrollView : UIScrollView

//: @end
@end

//: @interface NTESPageView ()
@interface StreetwiseView ()
{
...
- (void)layoutSubviews
{
    ...
    if (self.pageViewDelegate && [self.pageViewDelegate respondsToSelector:@selector(sessionAnimation)])
    {
        //: animation = [self.pageViewDelegate needScrollAnimation];
        animation = [self.pageViewDelegate sessionAnimation];
    }
...
- (void)roundItem
{
    ...
    if (_scrollView == nil)
    {
        ...
        _scrollView = [[CommentScrollView alloc]initWithFrame:self.bounds];
...
- (void)objectColor: (NSInteger)page
{
...
- (NSInteger)pageBoundMinGeorgiaHomeBoyInwardMessage:(NSInteger)value last:(NSInteger)min twentyFourHourPeriod:(NSInteger)max{
    //: if (max < min) {
    if (max < min) {
        //: max = min;
        max = min;
...
- (void)box:(NSInteger)currentPage
{
    ...
    NSInteger first = [self pageBoundMinGeorgiaHomeBoyInwardMessage:currentPage - 1 last:0 twentyFourHourPeriod:count - 1];
    //: NSInteger last = [self pageInBound:currentPage + 1 min:0 max:count - 1];
    NSInteger last = [self pageBoundMinGeorgiaHomeBoyInwardMessage:currentPage + 1 last:0 twentyFourHourPeriod:count - 1];
    ...
    for (NSUInteger index = 0; index < count; index++)
    {
        ...
        if (NSLocationInRange(index, range))
        {
            ...
            if (![obj isKindOfClass:[UIView class]])
            {
                ...
                if (_dataSource && [_dataSource respondsToSelector:@selector(provider:technicality:)])
                {
                    //: UIView *view = [_dataSource pageView:self viewInPage:index];
                    UIView *view = [_dataSource provider:self technicality:index];
...
- (void)create
{
    ...
    if(_dataSource && [_dataSource respondsToSelector:@selector(replied:)])
    {
        //: numberOfPages = [_dataSource numberOfPages:self];
        numberOfPages = [_dataSource replied:self];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(importantsed:)])
    {
        //: [_pageViewDelegate pageViewDidScroll:self];
        [_pageViewDelegate importantsed:self];
    }
}
...
- (void)per
{
    ...
    if (_pageViewDelegate && [_pageViewDelegate respondsToSelector:@selector(example:data:graduatedTableFor:)])
    {
        //: [_pageViewDelegate pageViewScrollEnd:self
        [_pageViewDelegate example:self
                                //: currentIndex:_currentPage
                                data:_currentPage
                                  //: totolPages:[_pages count]];
                                  graduatedTableFor:[_pages count]];
    }
}
...
- (BOOL)outside:(UIGestureRecognizer *)gestureRecognizer postOn:(nonnull UIGestureRecognizer *)otherGestureRecognizer
{
