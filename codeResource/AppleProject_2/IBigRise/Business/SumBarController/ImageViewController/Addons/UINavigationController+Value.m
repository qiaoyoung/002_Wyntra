...
// __CLOSE_PRINT__
//
//  UINavigationController+Value.m
//  NIM
//
...
#import "UINavigationController+Value.h"
...
@implementation UINavigationController (Value)
...
+ (void)load{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        swizzling_exchangeMethod([UINavigationController class] ,@selector(supportedInterfaceOrientations), @selector(prevalenceImage));
        //: swizzling_exchangeMethod([UINavigationController class] ,@selector(shouldAutorotate), @selector(swizzling_shouldAutorotate));
        swizzling_exchangeMethod([UINavigationController class] ,@selector(shouldAutorotate), @selector(userMy));
    //: });
    });
}
...
- (UIInterfaceOrientationMask)prevalenceImage{
