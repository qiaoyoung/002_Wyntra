...
//: #import "DataExtensionHelper.h"
#import "DataExtensionHelper.h"

//: NSString *const NTESSubscribeNetState = @"net_state";
NSString *const kConst_stateContent = @"net_state";

//: NSString *const NTESSubscribeOnlineState = @"online_state";
NSString *const kConst_modeData = @"online_state";
...
@interface InputManager()<NIMEventSubscribeManagerDelegate,NIMConversationManagerDelegate,NIMLoginManagerDelegate,NIMUserManagerDelegate>
...
+ (instancetype)pressed
{
    ...
    _dispatch_once(&onceToken, ^{
        ...
        instance = [[InputManager alloc] init];
    //: });
    });
    //: return instance;
    return instance;
...
- (void)stateReply:(NSString *)userId
{
    ...
    NIMSubscribeRequest *request = [self questionEmpty];
...
- (void)shouldState
{
    ...
    NSDictionary *ext = @{
                            ...
                            kConst_stateContent : @([CenterValue displayEnable].rangeCustom),
                            //: NTESSubscribeOnlineState : @(NTESOnlineStateNormal), 
                            kConst_modeData : @(NTESOnlineStateNormal), //移动端永远在线
                          //: };
                          };
    //: [event setExt:[ext nimkit_jsonString]];
    [event setExt:[ext distance]];
...
- (void)blockState
{
    ...
    [_subscribeIds addObjectsFromArray:self.at.allObjects];
    //: [_subscribeIds addObjectsFromArray:self.contactUserIds.allObjects];
    [_subscribeIds addObjectsFromArray:self.disable.allObjects];

    //: [self subscribeNextMembers:_subscribeIds.allObjects];
    [self straddleLight:_subscribeIds.allObjects];
}

//: - (void)subscribeNextMembers:(NSArray *)ids
- (void)straddleLight:(NSArray *)ids
{
    //: if (!ids.count) {
    if (!ids.count) {
        //: return;
    ...
    [[NIMSDK sharedSDK].subscribeManager subscribeEvent:request completion:^(NSError * _Nullable error, NSArray * _Nullable failedPublishers) {
        ...
        if (members.count) {
            ...
            [weakSelf straddleLight:members];
        }
    //: }];
    }];
}
...
- (void)onFriendChanged:(NIMUser *)user
{
    ...
    if (!isMyFriend && ![self.at containsObject:user.userId]) {
              ...
              totalUnreadCount:(NSInteger)totalUnreadCount
{
    ...
    if (recentSession.session.sessionType == NIMSessionTypeP2P && ![self.disable containsObject:recentSession.session.sessionId]) {
...
- (NIMSubscribeRequest *)questionEmpty
{
