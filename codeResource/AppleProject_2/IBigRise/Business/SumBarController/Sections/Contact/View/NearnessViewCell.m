...
@implementation NearnessViewCell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self) {
        ...
        [_avatarImageView addTarget:self action:@selector(keyAvatar:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)preserveMember:(QueryItemHide *)member{
    ...
    self.textLabel.text = [ComponentUtil showToExecutiveSession:member.info.infoId specialSession:nil];
    ...
    [_avatarImageView towardNeed:url user:member.info.avatarImage can:SDWebImageRetryFailed];
}
