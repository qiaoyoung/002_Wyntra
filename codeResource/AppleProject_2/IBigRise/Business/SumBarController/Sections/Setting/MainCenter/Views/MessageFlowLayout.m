...
static CGFloat const kConst_redText = 0.1;
//: static CGFloat const MIN_PRESS_TO_BEGIN_EDITING_DURATION = 0.6;
static CGFloat const kConst_transformName = 0.6;
...
@interface MessageFlowLayout () <UIGestureRecognizerDelegate>

//: @property (nonatomic,readonly) id<LxGridViewDataSource> dataSource;
@property (nonatomic,readonly) id<ChangeSource> dataSource;
//: @property (nonatomic,readonly) id<LxGridViewDelegateFlowLayout> delegate;
@property (nonatomic,readonly) id<BeReloadLayout> delegate;
...
- (void)image
{
    ...
    _longPressGestureRecognizer = [[UILongPressGestureRecognizer alloc]initWithTarget:self action:@selector(appWithTriggerd:)];
    //: _longPressGestureRecognizer.cancelsTouchesInView = NO;
    _longPressGestureRecognizer.cancelsTouchesInView = NO;
    //: _longPressGestureRecognizer.minimumPressDuration = PRESS_TO_MOVE_MIN_DURATION;
    _longPressGestureRecognizer.minimumPressDuration = kConst_redText;
    ...
    _panGestureRecognizer = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(immobilizes:)];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(coverred:) name:UIApplicationWillResignActiveNotification object:nil];
}
...
- (void)appWithTriggerd:(UILongPressGestureRecognizer *)longPress
{
    ...
    switch (longPress.state) {
        ...
        case UIGestureRecognizerStateBegan:
        {
            ...
            if (_displayLink == nil) {
                ...
                _displayLink = [CADisplayLink displayLinkWithTarget:self selector:@selector(messageTo:)];
                ...
                _remainSecondsToBeginEditing = kConst_transformName;
            }
            ...
            if ([self.delegate respondsToSelector:@selector(max:bottomShouldIcon:message:)]) {
                //: [self.delegate collectionView:self.collectionView layout:self willBeginDraggingItemAtIndexPath:_movingItemIndexPath];
                [self.delegate max:self.collectionView bottomShouldIcon:self message:_movingItemIndexPath];
            }
            ...
            SignalingReusableView *sourceCell = (SignalingReusableView *)sourceCollectionViewCell;
            ...
            UIView * highlightedSnapshotView = [sourceCell paradigm];
            ...
            UIView * snapshotView = [sourceCell paradigm];
            ...
            [UIView animateWithDuration:0
                             ...
                             completion:^(BOOL finished) {
                                 ...
                                 if (strongSelf) {
                                     ...
                                     if ([strongSelf.delegate respondsToSelector:@selector(name:view:bottom:)]) {
                                         ...
                                         [strongSelf.delegate name:strongSelf.collectionView view:strongSelf bottom:self->_movingItemIndexPath];
                                     }
                                 }
                             //: }];
                             }];
        ...
        case UIGestureRecognizerStateCancelled:
        {
            ...
            if (movingItemIndexPath) {
                ...
                if ([self.delegate respondsToSelector:@selector(titleContainer:umbra:lapsePath:)]) {
                    //: [self.delegate collectionView:self.collectionView layout:self willEndDraggingItemAtIndexPath:movingItemIndexPath];
                    [self.delegate titleContainer:self.collectionView umbra:self lapsePath:movingItemIndexPath];
                }
                ...
                [UIView animateWithDuration:0.2
                                 ...
                                 completion:^(BOOL finished) {
                                     ...
                                     if (strongSelf) {
                                         ...
                                         if ([strongSelf.delegate respondsToSelector:@selector(content:collectionTingAmbages:message:)]) {
                                             ...
                                             [strongSelf.delegate content:strongSelf.collectionView collectionTingAmbages:strongSelf message:movingItemIndexPath];
                                         }
                                     }
                                 //: }];
                                 }];
...
- (void)immobilizes:(UIPanGestureRecognizer *)pan
{
    //: switch (pan.state) {
    switch (pan.state) {
        //: case UIGestureRecognizerStatePossible:
        ...
        case UIGestureRecognizerStateChanged:
        {
            ...
            if ([self.dataSource respondsToSelector:@selector(array:sphere:info:)] && [self.dataSource array:self.collectionView sphere:sourceIndexPath info:destinationIndexPath] == NO) {
                //: return;
                return;
            }

            //: if ([self.dataSource respondsToSelector:@selector(collectionView:itemAtIndexPath:willMoveToIndexPath:)]) {
            if ([self.dataSource respondsToSelector:@selector(byLabel:voice:input:)]) {
                ...
                [self.dataSource byLabel:self.collectionView voice:sourceIndexPath input:destinationIndexPath];
            }
            ...
            //: } completion:^(BOOL finished) {
                ...
                if ([strongSelf.dataSource respondsToSelector:@selector(emotion:view:key:)]) {
                    ...
                    [strongSelf.dataSource emotion:strongSelf.collectionView view:sourceIndexPath key:destinationIndexPath];
                }
            //: }];
            }];
        }
...
- (void)messageTo:(CADisplayLink *)displayLink
{
...
- (void)coverred:(NSNotification *)notificaiton
{
