...
- (EdgeImageUnit *)containerAdd: (NSString *)source;
//: @end
@end


//: @implementation NTESSpellingCenter
@implementation ItemTing
//: + (NTESSpellingCenter *)sharedCenter
+ (ItemTing *)collectiveTingMiddle
{
    //: static NTESSpellingCenter *instance = nil;
    static ItemTing *instance = nil;
    //: static dispatch_once_t onceToken;
    static dispatch_once_t onceToken;
    //: _dispatch_once(&onceToken, ^{
    _dispatch_once(&onceToken, ^{
        //: instance = [[NTESSpellingCenter alloc]init];
        instance = [[ItemTing alloc]init];
    //: });
    });
    //: return instance;
    return instance;
...
- (EdgeImageUnit *)unwished:(NSString *)source
{
    //: if ([source length] == 0)
    if ([source length] == 0)
    {
    ...
    EdgeImageUnit *spellingUnit = nil;
    //: @synchronized(self)
    @synchronized(self)
    {
        //: SpellingUnit *unit = [_spellingCache objectForKey:source];
        EdgeImageUnit *unit = [_spellingCache objectForKey:source];
        //: if (!unit)
        if (!unit)
        {
            //: unit = [self calcSpellingOfString:source];
            unit = [self containerAdd:source];
...
- (NSString *)instance:(NSString *)input
{
    //: SpellingUnit *unit = [self spellingForString:input];
    EdgeImageUnit *unit = [self unwished:input];
...
- (EdgeImageUnit *)containerAdd:(NSString *)source
{
    ...
    for (NSInteger i = 0; i < [source length]; i++)
    {
        ...
        NSString *pinyin = [[CenterShould tag] cell:word];
    ...
    EdgeImageUnit *unit = [[EdgeImageUnit alloc]init];
