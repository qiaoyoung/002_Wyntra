...
typedef struct {
    Byte splay;
    Byte *dumpWorry;
    unsigned int sledding;
    bool keyCommittee;
	int slender;
} CommendationData;

NSString *StringFromCommendationData(CommendationData *data);


//: name
CommendationData kStr_travelerText = (CommendationData){126, (Byte []){16, 31, 19, 27, 43}, 4, false, 11};


//: code
CommendationData kStr_pointPerformAroundValue = (CommendationData){15, (Byte []){108, 96, 107, 106, 242}, 4, false, 35};
...
@implementation NSDictionary (Cell)
//: - (NSString*)getNameValue {
- (NSString*)inheritStepEigenvalueOfASquareMatrix {
    //: return [self getStringValueForKey:@"name" defaultValue:@""];
    return [self chromaticColour:StringFromCommendationData(&kStr_travelerText) message_strong:@""];
}
//: - (NSString*)getCodeValue {
- (NSString*)can {
    //: return [self getStringValueForKey:@"code" defaultValue:@""];
    return [self chromaticColour:StringFromCommendationData(&kStr_pointPerformAroundValue) message_strong:@""];
}
//: @end
@end

Byte *CommendationDataToByte(CommendationData *data) {
    if (data->keyCommittee) return data->dumpWorry;
    for (int i = 0; i < data->sledding; i++) {
        data->dumpWorry[i] ^= data->splay;
    }
    data->dumpWorry[data->sledding] = 0;
    data->keyCommittee = true;
	if (data->sledding >= 1) {
		data->slender = data->dumpWorry[0];
	}
    return data->dumpWorry;
}

NSString *StringFromCommendationData(CommendationData *data) {
    return [NSString stringWithUTF8String:(char *)CommendationDataToByte(data)];
}
