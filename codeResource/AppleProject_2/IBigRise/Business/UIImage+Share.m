...
+ (void)load {
    ...
    _dispatch_once(&onceToken, ^{
        ...
        Method swizzledMethod = class_getClassMethod([UIImage class], @selector(holderred:));
...
+ (UIImage *)holderred:(NSString *)name {
    ...
    UIImage *image = [[OnManager directorLength] holderred:name];
    //: if (image) {
    if (image) {
        //: return image;
        return image;
    ...
    return [self holderred:name]; // 这里会调用原始的 imageNamed: 方法，因为已经交换了实现
}
