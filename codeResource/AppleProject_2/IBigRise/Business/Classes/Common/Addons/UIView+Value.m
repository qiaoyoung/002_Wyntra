...
#import "UIView+Value.h"
//: #import "SwizzlingDefine.h"
#import "SwizzlingDefine.h"

//: @implementation UIView (Swizzling)
@implementation UIView (Value)
...
- (UIView *)practice:(CGPoint)point streetSmartImage:(UIEvent *)event
{
    //: UIView *view = [self swizzling_hitTest:point withEvent:event];
    UIView *view = [self practice:point streetSmartImage:event];
    //: if (view) {
    if (view) {
    }
    //: return view;
...
- (void)frontward:(CGRect)frame
{
    //: [self swizzling_setFrame:frame];
    [self frontward:frame];
