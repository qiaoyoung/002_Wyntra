...
@interface ImageViewController () <SightReload>

//: @property (nonatomic, strong) CustomTabBar *customTabBar;
@property (nonatomic, strong) CountView *customTabBar;
...
- (void)heritage {
    ...
    self.customTabBar = [[CountView alloc] init];
...
- (void)viewDidLayoutSubviews {
    ...
    //: CGFloat tabBarHeight = (({ BOOL isPhoneX = NO; if (@available(iOS 11.0, *)) { isPhoneX = [[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0.0; } isPhoneX;}) ? 49 + 35 : 49);
    CGFloat tabBarHeight = (({ BOOL isPhoneX = NO; if (@available(iOS 11.0, *)) { isPhoneX = [[UIApplication sharedApplication] delegate].window.safeAreaInsets.bottom > 0.0; } isPhoneX;}) ? 49 + 35 : 49);
...
- (void)send:(CountView *)tabBar tabOff:(NSInteger)index {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(image:threadMessage:)]) {
        //: BOOL shouldSelect = [self.delegate customTabBarController:self shouldSelectViewController:self.viewControllers[index]];
        BOOL shouldSelect = [self.delegate image:self threadMessage:self.viewControllers[index]];
        //: if (!shouldSelect) {
        if (!shouldSelect) {
            //: return;
            return;
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(key:should:)]) {
        //: [self.delegate customTabBarController:self didSelectViewController:self.selectedViewController];
        [self.delegate key:self should:self.selectedViewController];
    }
}
...
- (void)value:(NSString *)badgeValue send:(NSInteger)index {
    ...
    if (index < self.viewControllers.count) {
        ...
        [self.customTabBar scaleAcross:badgeValue must:index];
    }
}
