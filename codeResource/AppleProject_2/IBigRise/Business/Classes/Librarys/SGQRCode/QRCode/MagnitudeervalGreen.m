...
// __M_A_C_R_O__
//: #import "SGScanCode.h"
#import "MagnitudeervalGreen.h"
//: #import <AVFoundation/AVFoundation.h>
#import <AVFoundation/AVFoundation.h>
//: #import "SGSoundEffect.h"
#import "RadiogramName.h"
...
@interface MagnitudeervalGreen () <AVCaptureMetadataOutputObjectsDelegate, AVCaptureVideoDataOutputSampleBufferDelegate>
{
    //: SGSoundEffect *soundEffect;
    RadiogramName *soundEffect;
}
...
@implementation MagnitudeervalGreen
...
- (void)setDelegate:(id<InputDelegate>)delegate {
    //: _delegate = delegate;
    _delegate = delegate;
...
- (void)setSampleBufferDelegate:(id<ScanDelegate>)sampleBufferDelegate {
...
- (void)sizeCompletion:(UIImage *)image show:(void (^)(NSString *result))completion {
...
- (void)userLabel:(NSString *)name {
    ...
    NSString *voicePath = [[[OnManager directorLength] event] stringByAppendingPathComponent:[NSString stringWithFormat:@"%@", name]];
    ...
    soundEffect = [RadiogramName messageSuccess:voicePath];
    //: [soundEffect play];
    [soundEffect byWe];
}
...
- (AVCaptureSession *)session {
    ...
    if (!_session) {
        ...
        _session.sessionPreset = [self redPreset];
    }
    //: return _session;
    return _session;
}
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection {
    ...
    if (metadataObjects != nil && metadataObjects.count > 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (self.delegate && [self.delegate respondsToSelector:@selector(gray:kitView:)]) {
                //: [self.delegate scanCode:self result:resultString];
                [self.delegate gray:self kitView:resultString];
            }
        //: });
        });
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (self.sampleBufferDelegate && [self.sampleBufferDelegate respondsToSelector:@selector(scan:beforeImage:)]) {
            //: [self.sampleBufferDelegate scanCode:self brightness:brightnessValue];
            [self.sampleBufferDelegate scan:self beforeImage:brightnessValue];
        }
    //: });
    });
}
