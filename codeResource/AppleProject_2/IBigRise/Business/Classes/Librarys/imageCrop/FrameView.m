...
NSString *StringFromConstituentData(Byte *data);        


//: frame
Byte kStr_raceValue[] = {93, 5, 47, 5, 127, 55, 67, 50, 62, 54, 149};


//: center
Byte kStr_sayBillContent[] = {97, 6, 18, 11, 132, 200, 229, 3, 11, 204, 116, 81, 83, 92, 98, 83, 96, 66};
...
- (UIImage *)with:(CGRect)rect
{

    //: UIImage *fixedImage = [self fixOrientation];
    UIImage *fixedImage = [self replyInfo];
...
- (instancetype)initWithStreetSmart:(CGRect)frame with: (UIColor *)lineColor line: (CGFloat)lineWidth {
...
@interface ChangeEdgeView : UIView
...
@implementation ChangeEdgeView
//: - (instancetype)initWithLineWidth: (CGFloat)lineWidth lineHeight: (CGFloat)lineHeight lineColor: (UIColor *)lineColor {
- (instancetype)initWithNameColor: (CGFloat)lineWidth remote: (CGFloat)lineHeight screen: (UIColor *)lineColor {
...
@property (strong, nonatomic) TeamView *topLeftCorner;
//: @property (strong, nonatomic) CornerView *topRightCorner;
@property (strong, nonatomic) TeamView *topRightCorner;
//: @property (strong, nonatomic) CornerView *bottomLeftCorner;
@property (strong, nonatomic) TeamView *bottomLeftCorner;
//: @property (strong, nonatomic) CornerView *bottomRightCorner;
@property (strong, nonatomic) TeamView *bottomRightCorner;
//: @property (strong, nonatomic) CropAreaView *cropAreaView;
@property (strong, nonatomic) YieldView *cropAreaView;
...
@property (strong, nonatomic) ChangeEdgeView *topMidLine;
//: @property (strong, nonatomic) MidLineView *leftMidLine;
@property (strong, nonatomic) ChangeEdgeView *leftMidLine;
//: @property (strong, nonatomic) MidLineView *bottomMidLine;
@property (strong, nonatomic) ChangeEdgeView *bottomMidLine;
//: @property (strong, nonatomic) MidLineView *rightMidLine;
@property (strong, nonatomic) ChangeEdgeView *rightMidLine;
...
- (void)dealloc {
    ...
    [_cropAreaView removeObserver: self forKeyPath: StringFromConstituentData(kStr_raceValue)];
    //: [_cropAreaView removeObserver: self forKeyPath: @"center"];
    [_cropAreaView removeObserver: self forKeyPath: StringFromConstituentData(kStr_sayBillContent)];
    //: [_imageView removeObserver: self forKeyPath: @"frame"];
    [_imageView removeObserver: self forKeyPath: StringFromConstituentData(kStr_raceValue)];
...
- (void)my {
    ...
    _cropAreaView = [[YieldView alloc] init];
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"frame"
                    forKeyPath: StringFromConstituentData(kStr_raceValue)
    ...
    [_cropAreaView addObserver: self
                    //: forKeyPath: @"center"
                    forKeyPath: StringFromConstituentData(kStr_sayBillContent)
                       //: options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       options: NSKeyValueObservingOptionNew | NSKeyValueObservingOptionInitial
                       //: context: NULL];
                       context: NULL];
...
- (void)system {
    ...
    _topLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(modePan:)];
    //: _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _topRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(modePan:)];
    //: _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomLeftPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(modePan:)];
    //: _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCornerPan:)];
    _bottomRightPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(modePan:)];
    //: _cropAreaPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(handleCropAreaPan:)];
    _cropAreaPan = [[UIPanGestureRecognizer alloc]initWithTarget: self action: @selector(toed:)];
...
- (void)shouldPinch: (UIPinchGestureRecognizer *)pinchGesture {
    ...
    switch (pinchGesture.state) {
        ...
        case UIGestureRecognizerStateChanged: {
            ...
            [self firstFactor: pinchGesture.scale];
            //: break;
            break;
        }
        //: default:
...
- (void)toed: (UIPanGestureRecognizer *)panGesture {
...
- (void)appLine: (UIPanGestureRecognizer *)panGesture {

    //: MidLineView *midLineView = (MidLineView *)panGesture.view;
    ChangeEdgeView *midLineView = (ChangeEdgeView *)panGesture.view;
    //: switch (panGesture.state) {
    switch (panGesture.state) {
        //: case UIGestureRecognizerStateBegan: {
        case UIGestureRecognizerStateBegan: {
...
- (void)modePan: (UIPanGestureRecognizer *)panGesture {

    //: CornerView *panView = (CornerView *)panGesture.view;
    TeamView *panView = (TeamView *)panGesture.view;
    //: CornerView *relativeViewX = panView.relativeViewX;
    TeamView *relativeViewX = panView.relativeViewX;
    //: CornerView *relativeViewY = panView.relativeViewY;
    TeamView *relativeViewY = panView.relativeViewY;
...
- (void)data {
    ...
    [_topLeftCorner mark: _cropAreaCornerWidth beLimit: _cropAreaCornerHeight];
    //: [_topRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_topRightCorner mark: _cropAreaCornerWidth beLimit: _cropAreaCornerHeight];
    //: [_bottomLeftCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomLeftCorner mark: _cropAreaCornerWidth beLimit: _cropAreaCornerHeight];
    //: [_bottomRightCorner updateSizeWithWidth: _cropAreaCornerWidth height: _cropAreaCornerHeight];
    [_bottomRightCorner mark: _cropAreaCornerWidth beLimit: _cropAreaCornerHeight];
...
- (void)conversation {
    ...
    _topLeftCorner = [[TeamView alloc]initWithStreetSmart: CGRectMake(0, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) with:_cropAreaCornerLineColor line: _cropAreaCornerLineWidth];
    ...
    _topRightCorner = [[TeamView alloc]initWithStreetSmart: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, 0, _cropAreaCornerWidth, _cropAreaCornerHeight) with: _cropAreaCornerLineColor line: _cropAreaCornerLineWidth];
    ...
    _bottomLeftCorner = [[TeamView alloc]initWithStreetSmart: CGRectMake(0, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) with: _cropAreaCornerLineColor line: _cropAreaCornerLineWidth];
    ...
    _bottomRightCorner = [[TeamView alloc]initWithStreetSmart: CGRectMake(CGRectGetWidth(_imageView.bounds) - _cropAreaCornerWidth, CGRectGetHeight(_imageView.bounds) - _cropAreaCornerHeight, _cropAreaCornerWidth, _cropAreaCornerHeight) with: _cropAreaCornerLineColor line: _cropAreaCornerLineWidth];
...
- (void)godspeed {
    ...
    _topMidLine = [[ChangeEdgeView alloc]initWithNameColor: _cropAreaMidLineWidth remote: _cropAreaMidLineHeight screen: _cropAreaMidLineColor];
    ...
    _bottomMidLine = [[ChangeEdgeView alloc]initWithNameColor: _cropAreaMidLineWidth remote: _cropAreaMidLineHeight screen: _cropAreaMidLineColor];
    ...
    _leftMidLine = [[ChangeEdgeView alloc]initWithNameColor: _cropAreaMidLineWidth remote: _cropAreaMidLineHeight screen: _cropAreaMidLineColor];
    ...
    _rightMidLine = [[ChangeEdgeView alloc]initWithNameColor: _cropAreaMidLineWidth remote: _cropAreaMidLineHeight screen: _cropAreaMidLineColor];
    ...
    _topMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(appLine:)];
    ...
    _bottomMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(appLine:)];
    ...
    _leftMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(appLine:)];
    ...
    _rightMidPan = [[UIPanGestureRecognizer alloc]initWithTarget:self action: @selector(appLine:)];
...
- (void)firstFactor: (CGFloat)scaleFactor {
...
- (void)setNeedScaleCrop:(BOOL)needScaleCrop {
    ...
    if(!_needScaleCrop && needScaleCrop) {
        ...
        _cropAreaPinch = [[UIPinchGestureRecognizer alloc]initWithTarget: self action:@selector(shouldPinch:)];
...
- (UIImage *)scheme {
    ...
    return [_toCropImage with: CGRectMake((CGRectGetMinX(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetMinY(_cropAreaView.frame) + _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetWidth(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor, (CGRectGetHeight(_cropAreaView.bounds) - 2 * _cropAreaBorderLineWidth) / scaleFactor)];

}
//: @end
@end

Byte * ConstituentDataToCache(Byte *data) {
    int universal = data[0];
    int forestMessage = data[1];
    Byte physicallyWall = data[2];
    int technology = data[3];
    if (!universal) return data + technology;
    for (int i = technology; i < technology + forestMessage; i++) {
        int value = data[i] + physicallyWall;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[technology + forestMessage] = 0;
    return data + technology;
}

NSString *StringFromConstituentData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)ConstituentDataToCache(data)];
}
