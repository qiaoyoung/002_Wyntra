...
NSString *StringFromRueGrossData(Byte *data);        


//: blurRadius
Byte kStr_payerTitle[] = {55, 10, 82, 13, 65, 91, 119, 30, 146, 167, 202, 148, 68, 16, 26, 35, 32, 0, 15, 18, 23, 35, 33, 253};


//: bounds
Byte kStr_willingnessEyebrowConscienceName[] = {46, 6, 67, 10, 90, 70, 192, 176, 4, 235, 31, 44, 50, 43, 33, 48, 83};


//: position
Byte kStr_keyNarrowData[] = {18, 8, 29, 10, 170, 224, 244, 93, 163, 1, 83, 82, 86, 76, 87, 76, 82, 81, 226};


//: linear
Byte kStr_classifyData[] = {56, 6, 19, 14, 93, 113, 147, 6, 155, 106, 35, 177, 188, 223, 89, 86, 91, 82, 78, 95, 111};


//: backgroundColor
Byte kStr_wheatTitle[] = {36, 15, 7, 14, 1, 253, 14, 57, 21, 197, 67, 40, 37, 18, 91, 90, 92, 100, 96, 107, 104, 110, 103, 93, 60, 104, 101, 104, 107, 233};
...
//
//  https://github.com/nicklockwood/Background
//
...
- (UIImage *)domain:(CGFloat)radius should:(NSUInteger)iterations input:(UIColor *)tintColor
{
...
@interface NameDropLayer: CALayer
...
+ (BOOL)needsDisplayForKey:(NSString *)key
{
    ...
    if ([@[StringFromRueGrossData(kStr_payerTitle), StringFromRueGrossData(kStr_willingnessEyebrowConscienceName), StringFromRueGrossData(kStr_keyNarrowData)] containsObject:key])
    {
        //: return YES;
        return YES;
    }
...
@implementation BlurEdge

//: + (instancetype)sharedInstance
+ (instancetype)receiveList
{
    //: static FXBlurScheduler *sharedInstance = nil;
    static BlurEdge *sharedInstance = nil;
    //: if (!sharedInstance)
    if (!sharedInstance)
    {
        //: sharedInstance = [[FXBlurScheduler alloc] init];
        sharedInstance = [[BlurEdge alloc] init];
    }
...
- (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    if (blurEnabled)
    {
        ...
        for (Background *view in self.views)
        {
...
- (void)bottomContainer
{
    ...
    if (self.blurEnabled && !self.updating && self.updatesEnabled > 0 && [self.views count])
    {
        ...
        for (NSUInteger i = self.viewIndex; i < [self.views count]; i++)
        {
            ...
            Background *view = self.views[i];
            //: if (view.dynamic && !view.hidden && view.window && [view shouldUpdate])
            if (view.dynamic && !view.hidden && view.window && [view next])
            {
        ...
        [self performSelector:@selector(bottomContainer)
                   //: withObject:nil
                   withObject:nil
...
+ (void)setBlurEnabled:(BOOL)blurEnabled
{
    ...
    [BlurEdge receiveList].blurEnabled = blurEnabled;
}
...
+ (void)device
{
    ...
    [[BlurEdge receiveList] telecasting];
}
...
- (void)imageButton
{
    ...
    if (!_blurRadiusSet) [self container].blurRadius = 40;
    ...
    self.layer.magnificationFilter = StringFromRueGrossData(kStr_classifyData); // kCAFilterLinear
...
- (void)setBlurRadius:(CGFloat)blurRadius
{
    ...
    [self container].blurRadius = blurRadius;
}
...
- (CGFloat)blurRadius
{
    ...
    return [self container].blurRadius;
}
...
- (NameDropLayer *)container
{
    //: return (FXBlurLayer *)self.layer;
    return (NameDropLayer *)self.layer;
}

//: - (FXBlurLayer *)blurPresentationLayer
- (NameDropLayer *)colorName
{
    //: FXBlurLayer *blurLayer = [self blurLayer];
    NameDropLayer *blurLayer = [self container];
    //: return (FXBlurLayer *)blurLayer.presentationLayer ?: blurLayer;
    return (NameDropLayer *)blurLayer.presentationLayer ?: blurLayer;
}
...
- (void)app
{
    ...
    if (self.window && self.dynamic && self.blurEnabled)
    {
        ...
        [[BlurEdge receiveList] add:self];
    }
    //: else
    else
    {
        //: [[FXBlurScheduler sharedInstance] removeView:self];
        [[BlurEdge receiveList] team:self];
    }
}
...
- (BOOL)next
{
    ...
    __strong CALayer *underlyingLayer = [self green];
    ...
    self.blurEnabled && [BlurEdge receiveList].blurEnabled &&
...
- (id<CAAction>)actionForLayer:(CALayer *)layer forKey:(NSString *)key
{
    ...
    if ([key isEqualToString:StringFromRueGrossData(kStr_payerTitle)])
    {
        //animations are enabled
        //: CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:@"backgroundColor"];
        CAAnimation *action = (CAAnimation *)[super actionForLayer:layer forKey:StringFromRueGrossData(kStr_wheatTitle)];
...
- (UIImage *)temp
{
    ...
    __strong NameDropLayer *blurLayer = [self colorName];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self green];
    ...
    NSArray *hiddenViews = [self searchion];
    //: [underlyingLayer renderInContext:context];
    [underlyingLayer renderInContext:context];
    //: [self restoreSuperviewAfterSnapshot:hiddenViews];
    [self title:hiddenViews];
...
- (NSArray *)searchion
{
    ...
    __strong CALayer *blurlayer = [self container];
    //: __strong CALayer *underlyingLayer = [self underlyingLayer];
    __strong CALayer *underlyingLayer = [self green];
...
- (void)title:(NSArray *)hiddenLayers
{
...
- (UIImage *)pin:(UIImage *)snapshot quantity:(CGFloat)blurRadius
{
    //: return [snapshot blurredImageWithRadius:blurRadius
    return [snapshot domain:blurRadius
                                 //: iterations:self.iterations
                                 should:self.iterations
                                  //: tintColor:self.tintColor];
                                  input:self.tintColor];
}

//: - (void)setLayerContents:(UIImage *)image
- (void)setPhonation:(UIImage *)image
{
...
- (void)session:(BOOL)async style:(void (^)())completion
{
    //: if ([self shouldUpdate])
    if ([self next])
    {
        //: UIImage *snapshot = [self snapshotOfUnderlyingView];
        UIImage *snapshot = [self temp];
        //: if (async)
        if (async)
        {
            ...
            dispatch_async(dispatch_get_global_queue((-2), 0), ^{
                ...
                UIImage *blurredImage = [self pin:snapshot quantity:self.blurRadius];
                //: dispatch_sync(dispatch_get_main_queue(), ^{
                dispatch_sync(dispatch_get_main_queue(), ^{

                    //: [self setLayerContents:blurredImage];
                    [self setPhonation:blurredImage];
        ...
        else
        {
            ...
            [self setPhonation:[self pin:snapshot quantity:[self colorName].blurRadius]];
            //: if (completion) completion();
            if (completion) completion();
        }
    }
...
Byte * RueGrossDataToCache(Byte *data) {
    int fingerMath = data[0];
    int galaxyMedia = data[1];
    Byte hostageCruiseIncreased = data[2];
    int impressionHim = data[3];
    if (!fingerMath) return data + impressionHim;
    for (int i = impressionHim; i < impressionHim + galaxyMedia; i++) {
        int value = data[i] + hostageCruiseIncreased;
        if (value > 255) {
            value -= 256;
        }
        data[i] = value;
    ...
    data[0] = 0;
    data[impressionHim + galaxyMedia] = 0;
    return data + impressionHim;
}

NSString *StringFromRueGrossData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)RueGrossDataToCache(data)];
}
