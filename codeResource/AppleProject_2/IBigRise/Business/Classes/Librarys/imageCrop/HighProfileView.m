...
static CGFloat const kConst_cropData = 0.75;
//: static BOOL const square = NO;
static BOOL const kConst_midLinkData = NO;
//: float IMAGE_MIN_HEIGHT = 400;
float kConst_rawCommentTitle = 400;
//: float IMAGE_MIN_WIDTH = 400;
float kConst_kitTitle = 400;
...
#pragma mark DropViewController implementation
...
- (void)viewDidLoad
{
    ...
    if (self){
        ...
        self.cropView = [[HighProfileView alloc] initWithCropLast:view path:self.blurredBackground];
...
- (void)highed:(id)sender
{
    ...
    if ([self.delegate respondsToSelector:@selector(be:publication:)])
    {
        ...
        [self.delegate be:self publication:cropped];
    }
...
#pragma mark TapView implementation
...
static CGFloat const kConst_maxTitleValue = 5;
...
@implementation HighProfileView
...
- (void)initSize {
    ...
    self.shadeView = [[BoundsView alloc] initWithFrame:subviewFrame];
    ...
    controlPointSize = kConst_maxTitleValue;
    ...
    topLeftPoint = [self max:SquareCGRectAtCenter(centerInView.x - initialCropAreaSize,
    ...
    bottomLeftPoint = [self max:SquareCGRectAtCenter(centerInView.x - initialCropAreaSize,
                                                                      //: centerInView.y + initialCropAreaSize,
                                                                      centerInView.y + initialCropAreaSize,
                                                                      //: controlPointSize)];
                                                                      controlPointSize)];
    ...
    bottomRightPoint = [self max:SquareCGRectAtCenter(centerInView.x + initialCropAreaSize,
    ...
    topRightPoint = [self max:SquareCGRectAtCenter(centerInView.x + initialCropAreaSize,
                                                                    //: centerInView.y - initialCropAreaSize, controlPointSize)];
                                                                    centerInView.y - initialCropAreaSize, controlPointSize)];

    //the "hole"
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self message];
    ...
    self.maskAlpha = kConst_cropData;
...
- (TapView*)max:(CGRect)frame {
    //: ControlPointView* point = [[ControlPointView alloc] initWithFrame:frame];
    TapView* point = [[TapView alloc] initWithFrame:frame];
    //: return point;
    return point;
}
...
- (void)textRight:(CGPoint)topLeft cleanlyPoint:(CGPoint)bottomLeft compartment:(CGPoint)bottomRight ofRight:(CGPoint)topRight {
...
- (void)dragged:(UIPanGestureRecognizer*)recognizer
{
    ...
    if (recognizer.state == UIGestureRecognizerStateBegan || multiDragPoint.lastCount != count) {
        ...
        if (count > 1)
            ...
            [self imageMagnitude:recognizer message:count];
        //: else
        else
    ...
    if (count > 1) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            [self withView:[recognizer locationOfTouch:i inView:self] edgeShow:(i == 0 ? dragViewOne : dragViewTwo)];
    ...
    //: } else {
        ...
        [self withView:[recognizer locationInView:self] edgeShow:dragViewOne];
    }
...
- (void)imageMagnitude:(UIPanGestureRecognizer*)recognizer message:(NSUInteger)count
{
    //: for (int i = 0; i < count; i++) {
    for (int i = 0; i < count; i++) {
        //: if (i == 0) {
        if (i == 0) {
            ...
            dragViewOne = [self data:[recognizer locationOfTouch:i inView:self]];
        ...
        //: } else {
            ...
            dragViewTwo = [self data:[recognizer locationOfTouch:i inView:self]];
...
- (void)text:(UIPanGestureRecognizer*)recognizer
{
    //: dragViewOne = [self checkHit:[recognizer locationInView:self]];
    dragViewOne = [self data:[recognizer locationInView:self]];
...
- (void)withView:(CGPoint)location edgeShow:(UIView*)view
{
    ...
    //: } else if (view == cropAreaView) {
        ...
        [self dateSession:location];
    }

    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self message];
    //: [self setCropAreaForViews:cropArea];
    [self setHide:cropArea];
}
...
- (CGSize)name:(CGPoint)dragLocation to:(CGPoint)draggedPoint nim:(CGPoint)oppositePoint {
...
- (void)analogDigitalConverter:(CGPoint)dragLocation {
    ...
    CGSize disp = [self name:dragLocation to:dragPoint.topLeftCenter nim:dragPoint.bottomRightCenter];
    ...
    if (width >= kConst_kitTitle && height < kConst_rawCommentTitle) {
        //: topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - IMAGE_MIN_HEIGHT) / self.imageScale);
        topLeft.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - kConst_rawCommentTitle) / self.imageScale);
        //: topRight.y = topLeft.y;
        topRight.y = topLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < kConst_kitTitle && height >= kConst_rawCommentTitle) {
        //: topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        topLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - kConst_kitTitle) / self.imageScale);
    ...
    [self textRight:topLeft cleanlyPoint:bottomLeft compartment:dragPoint.bottomRightCenter ofRight:topRight];
}
//: - (void)handleDragBottomLeft:(CGPoint)dragLocation {
- (void)withLeft:(CGPoint)dragLocation {
    ...
    CGSize disp = [self name:dragLocation to:dragPoint.bottomLeftCenter nim:dragPoint.topRightCenter];
    ...
    if (width >= kConst_kitTitle && height < kConst_rawCommentTitle) {
        ...
        bottomLeft.y = cropArea.origin.y + (kConst_rawCommentTitle / self.imageScale);
        //: bottomRight.y = bottomLeft.y;
        bottomRight.y = bottomLeft.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < kConst_kitTitle && height >= kConst_rawCommentTitle) {
        //: bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - IMAGE_MIN_WIDTH) / self.imageScale);
        bottomLeft.x = cropArea.origin.x + (((cropArea.size.width * self.imageScale) - kConst_kitTitle) / self.imageScale);
    ...
    [self textRight:topLeft cleanlyPoint:bottomLeft compartment:bottomRight ofRight:dragPoint.topRightCenter];
}

//: - (void)handleDragBottomRight:(CGPoint)dragLocation {
- (void)betweenQuick:(CGPoint)dragLocation {
    ...
    CGSize disp = [self name:dragLocation to:dragPoint.bottomRightCenter nim:dragPoint.topLeftCenter];
    ...
    if (width >= kConst_kitTitle && height < kConst_rawCommentTitle) {
        ...
        bottomRight.y = cropArea.origin.y + (kConst_rawCommentTitle / self.imageScale);
        //: bottomLeft.y = bottomRight.y;
        bottomLeft.y = bottomRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < kConst_kitTitle && height >= kConst_rawCommentTitle) {
        //: bottomRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        bottomRight.x = cropArea.origin.x + (kConst_kitTitle / self.imageScale);
    ...
    [self textRight:dragPoint.topLeftCenter cleanlyPoint:bottomLeft compartment:bottomRight ofRight:topRight];
}

//: - (void)handleDragTopRight:(CGPoint)dragLocation {
- (void)bottom:(CGPoint)dragLocation {
    ...
    CGSize disp = [self name:dragLocation to:dragPoint.topRightCenter nim:dragPoint.bottomLeftCenter];
    ...
    if (width >= kConst_kitTitle && height < kConst_rawCommentTitle) {
        ...
        topRight.y = cropArea.origin.y + (((cropArea.size.height * self.imageScale) - kConst_rawCommentTitle) / self.imageScale);
        //: topLeft.y = topRight.y;
        topLeft.y = topRight.y;
    //: } else if (width < IMAGE_MIN_WIDTH && height >= IMAGE_MIN_HEIGHT) {
    } else if (width < kConst_kitTitle && height >= kConst_rawCommentTitle) {
        //: topRight.x = cropArea.origin.x + (IMAGE_MIN_WIDTH / self.imageScale);
        topRight.x = cropArea.origin.x + (kConst_kitTitle / self.imageScale);
    ...
    [self textRight:topLeft cleanlyPoint:dragPoint.bottomLeftCenter compartment:bottomRight ofRight:topRight];
}

//: - (void)handleDragCropArea:(CGPoint)dragLocation {
- (void)dateSession:(CGPoint)dragLocation {
    //: CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    CGFloat dX = dragLocation.x - dragPoint.dragStart.x;
    //: CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
    CGFloat dY = dragLocation.y - dragPoint.dragStart.y;
...
- (void)setImage:(UIImage *)image {
    ...
    if (self.blurred) {
        ...
        blur = [image domain:30 should:1 input:[UIColor blackColor]];
    //: } else {
    } else {
        //: blur = [image blurredImageWithRadius:0 iterations:1 tintColor:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
        blur = [image domain:0 should:1 input:[UIColor colorWithRed:0.5 green:0.5 blue:0.5 alpha:1.0]];
    }
    ...
    [self textRight:topLeftPoint.center cleanlyPoint:bottomLeftPoint.center compartment:bottomRightPoint.center ofRight:topRightPoint.center];
    //: CGRect cropArea = [self cropAreaFromControlPoints];
    CGRect cropArea = [self message];
    //: cropAreaView.frame = cropArea;
    cropAreaView.frame = cropArea;
