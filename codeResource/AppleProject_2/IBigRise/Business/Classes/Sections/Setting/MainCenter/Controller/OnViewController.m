...
NSString *StringFromMenuLengthData(Byte *data);


//: #612CF9
Byte kStr_timeText[] = {78, 7, 3, 57, 70, 67, 50, 49, 54, 35, 112};


//: feedback_activity_title
Byte kStr_messageShouldData[] = {56, 23, 4, 124, 101, 108, 116, 105, 116, 95, 121, 116, 105, 118, 105, 116, 99, 97, 95, 107, 99, 97, 98, 100, 101, 101, 102, 209};


//: report_info
Byte kStr_legislativeName[] = {39, 11, 9, 64, 44, 150, 39, 131, 211, 111, 102, 110, 105, 95, 116, 114, 111, 112, 101, 114, 158};


//: common_bg
Byte kStr_coverImageContent[] = {46, 9, 7, 205, 111, 102, 238, 103, 98, 95, 110, 111, 109, 109, 111, 99, 61};


//: contact
Byte kStr_vasData[] = {63, 7, 8, 149, 84, 29, 238, 190, 116, 99, 97, 116, 110, 111, 99, 76};


//: #999999
Byte kStr_sleeveData[] = {64, 7, 7, 75, 5, 89, 128, 57, 57, 57, 57, 57, 57, 35, 14};


//: #A148FF
Byte kStr_extensiveName[] = {86, 7, 5, 36, 223, 70, 70, 56, 52, 49, 65, 35, 86};


//: msg
Byte kStr_legallyValue[] = {62, 3, 12, 22, 116, 115, 218, 75, 26, 197, 64, 46, 103, 115, 109, 232};


//: #875FFA
Byte kStr_speciesValue[] = {80, 7, 10, 47, 214, 141, 143, 40, 37, 231, 65, 70, 70, 53, 55, 56, 35, 205};


//: back_arrow_bl
Byte kStr_itemUserName[] = {30, 13, 7, 214, 236, 61, 26, 108, 98, 95, 119, 111, 114, 114, 97, 95, 107, 99, 97, 98, 96};


//: icon_checkbox_selected
Byte kStr_markRedValue[] = {56, 22, 7, 184, 53, 216, 1, 100, 101, 116, 99, 101, 108, 101, 115, 95, 120, 111, 98, 107, 99, 101, 104, 99, 95, 110, 111, 99, 105, 139};


//: %lu/512
Byte kStr_coverMediaName[] = {35, 7, 6, 66, 5, 12, 50, 49, 53, 47, 117, 108, 37, 27};


//: image%lu
Byte kStr_mathValue[] = {29, 8, 12, 153, 62, 90, 74, 80, 217, 238, 171, 56, 117, 108, 37, 101, 103, 97, 109, 105, 7};


//: code
Byte kStr_outsideData[] = {23, 4, 3, 101, 100, 111, 99, 85};


//: group_info_activity_op_failed
Byte kStr_orationValue[] = {48, 29, 3, 100, 101, 108, 105, 97, 102, 95, 112, 111, 95, 121, 116, 105, 118, 105, 116, 99, 97, 95, 111, 102, 110, 105, 95, 112, 117, 111, 114, 103, 164};


//: AlbumAddBtn
Byte kStr_furnitureData[] = {16, 11, 11, 197, 66, 81, 245, 43, 202, 19, 68, 110, 116, 66, 100, 100, 65, 109, 117, 98, 108, 65, 215};


//: /other/feedback
Byte kStr_weepThereTitle[] = {46, 15, 3, 107, 99, 97, 98, 100, 101, 101, 102, 47, 114, 101, 104, 116, 111, 47, 125};


//: public.image
Byte kStr_vasValue[] = {32, 12, 11, 255, 240, 126, 58, 236, 182, 210, 245, 101, 103, 97, 109, 105, 46, 99, 105, 108, 98, 117, 112, 236};
...
@interface OnViewController ()<UITextViewDelegate,TZImagePickerControllerDelegate,UICollectionViewDataSource,UICollectionViewDelegate,UIImagePickerControllerDelegate,UINavigationControllerDelegate>
{
...
//    self.view.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"common_bg"]];
    ...
    bg.image = [UIImage imageNamed:StringFromMenuLengthData(kStr_coverImageContent)];
    ...
    [backButton setImage:[UIImage imageNamed:StringFromMenuLengthData(kStr_itemUserName)] forState:(UIControlStateNormal)];
    ...
    labtitle.text = [LabelCell tinkleKey:StringFromMenuLengthData(kStr_messageShouldData)];
    ...
    [submitButton setImage:[UIImage imageNamed:StringFromMenuLengthData(kStr_markRedValue)] forState:(UIControlStateNormal)];
    ...
    UIView *bgview = [[UIView alloc]initWithFrame:CGRectMake(15, (44.0f + [UIDevice encounter])+15, [[UIScreen mainScreen] bounds].size.width-30, 260)];
    ...
    _textView.placeholder = [LabelCell tinkleKey:StringFromMenuLengthData(kStr_legislativeName)];//@"请输入您的举报信息";
...
//    [self.view addSubview:_textView];
    ...
    self.numLabel.text = [NSString stringWithFormat:StringFromMenuLengthData(kStr_coverMediaName),(unsigned long)_textView.text.length];
...
//    }
    ...
    self.numLabel.text = [NSString stringWithFormat:StringFromMenuLengthData(kStr_coverMediaName),textView.text.length];
}
...
-(void)generalClick{
    ...
    dict[StringFromMenuLengthData(kStr_vasData)] = _textView.text;
    ...
    if (_selectedPhotos.count > 0) {
        ...
        [_selectedPhotos enumerateObjectsUsingBlock:^(UIImage *image , NSUInteger idx, BOOL * _Nonnull stop) {
            ...
            [nameArray addObject:[NSString stringWithFormat:StringFromMenuLengthData(kStr_mathValue),(unsigned long)(idx+1)]];
        //: }];
        }];
    }
//    [SVProgressHUD show];
    ...
    [ViewManager title:[NSString stringWithFormat:StringFromMenuLengthData(kStr_weepThereTitle)] with:dict change:array searched:nameArray successCell:^(NSProgress *progress) {
...
//        [SVProgressHUD dismiss];
        ...
        NSString *code = [resultDict length:StringFromMenuLengthData(kStr_outsideData)];
        //: NSString *msg = [resultDict newStringValueForKey:@"msg"];
        NSString *msg = [resultDict length:StringFromMenuLengthData(kStr_legallyValue)];
...
//        [SVProgressHUD dismiss];
        ...
        [SVProgressHUD text:[LabelCell tinkleKey:StringFromMenuLengthData(kStr_orationValue)]];
    //: }];
    }];
...
- (UILabel *)numLabel{
    ...
    if (!_numLabel) {
        ...
        _numLabel.textColor = [UIColor box:StringFromMenuLengthData(kStr_sleeveData)];
    }
    //: return _numLabel;
    return _numLabel;
}
...
- (UIImagePickerController *)imagePickerVc {
    ...
    if (_imagePickerVc == nil) {
        ...
        _imagePickerVc.navigationBar.barTintColor = [UIColor colorWithPatternImage:[VisualImageDisable length:[UIColor box:StringFromMenuLengthData(kStr_speciesValue)] canImage:[UIColor box:StringFromMenuLengthData(kStr_timeText)] labelMessage:SNLinearGradientDirectionLevel]];
        //: _imagePickerVc.navigationBar.tintColor = [UIColor colorWithHexString:@"#A148FF"];
        _imagePickerVc.navigationBar.tintColor = [UIColor box:StringFromMenuLengthData(kStr_extensiveName)];
    }
...
- (void)viewDidLayoutSubviews {
    ...
    self.collectionView.frame = CGRectMake(0, self.numLabel.add + self.numLabel.height, self.view.tz_width, self.view.tz_height-self.textView.height-50);
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.item == _selectedPhotos.count) {
        ...
        cell.imageView.image = [UIImage imageNamed:StringFromMenuLengthData(kStr_furnitureData)];
...
//        [[UIApplication sharedApplication] openURL:[NSURL URLWithString:UIApplicationOpenSettingsURLString]];
    ...
    //: } else {
        ...
        [self searchionController];
    }
}

// 调用相机
//: - (void)pushImagePickerController {
- (void)searchionController {
...
- (void)imagePickerController:(UIImagePickerController*)picker didFinishPickingMediaWithInfo:(NSDictionary *)info {
    ...
    if ([type isEqualToString:StringFromMenuLengthData(kStr_vasValue)]) {
        ...
        [[TZImageManager manager] savePhotoWithImage:image meta:meta location:nil completion:^(PHAsset *asset, NSError *error){
            ...
            //: } else {
                ...
                [self towardWith:assetModel.asset imageTo:image];
            }
        //: }];
        }];
    }
...
- (void)towardWith:(PHAsset *)asset imageTo:(UIImage *)image {
...
Byte * MenuLengthDataToCache(Byte *data) {
    int sumention = data[0];
    int docketCape = data[1];
    int successScale = data[2];
    if (!sumention) return data + successScale;
    for (int i = 0; i < docketCape / 2; i++) {
        int begin = successScale + i;
        int end = successScale + docketCape - i - 1;
        Byte temp = data[begin];
        data[begin] = data[end];
        data[end] = temp;
    }
    data[0] = 0;
    data[successScale + docketCape] = 0;
    return data + successScale;
}

NSString *StringFromMenuLengthData(Byte *data) {
    return [NSString stringWithUTF8String:(char *)MenuLengthDataToCache(data)];
}  
