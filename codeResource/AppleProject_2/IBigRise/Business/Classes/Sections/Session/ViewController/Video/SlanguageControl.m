...
@implementation SlanguageControl
{
    //: CGRect _frame;
    CGRect _frame;
}
...
- (void)currentDoing{
    ...
    self.thumb.center = [self getValue:_value];
...
- (CGPoint)getValue:(CGFloat)value{
...
- (void)setBufferProgress:(CGFloat)bufferProgress{
    ...
    bufferProgress = [self sub:bufferProgress];
...
- (void)sizeView:(UIImage *)thumbImage before:(UIControlState)state{
...
- (void)colorChanged:(BOOL)isFullScreen{
    //: _frame = self.bounds;
    _frame = self.bounds;
    //: [self creatUI];
    [self currentDoing];
