...
#import "ValueTitlePerson.h"
//: #import "NSString+LJExtension.h"
#import "NSString+Image.h"

//: @implementation ZZZContactPerson
@implementation ValueTitlePerson

//: - (instancetype)initWithCNContact:(CNContact *)contact {
- (instancetype)initWithNearness:(CNContact *)contact {
    //: self = [super init];
    self = [super init];
    //: if (self)
    if (self)
    {
        ...
        if ([contact isKeyAvailable:CNContactPhoneNumbersKey])
        {
            ...
            for (CNLabeledValue *labeledValue in contact.phoneNumbers)
            {
                ...
                if ([SelectMarginContainer add:labeledValue]) {
                    //: NIMContactPhone *phoneModel = [[NIMContactPhone alloc] initWithLabeledValue:labeledValue];
                    SelectMarginContainer *phoneModel = [[SelectMarginContainer alloc] initWithComplete:labeledValue];
...
@implementation SelectMarginContainer

//: - (instancetype)initWithLabeledValue:(CNLabeledValue *)labeledValue {
- (instancetype)initWithComplete:(CNLabeledValue *)labeledValue {
    //: self = [super init];
    self = [super init];
    //: if (self)
    if (self)
    {
        ...
        self.phone = [NSString m:phoneNumber];
...
+ (BOOL)add:(CNLabeledValue *)labeledValue {
    //: CNPhoneNumber *phoneValue = labeledValue.value;
    CNPhoneNumber *phoneValue = labeledValue.value;
    //: NSString *phoneNumber = phoneValue.stringValue;
    NSString *phoneNumber = phoneValue.stringValue;
    //: NSString *userful = [NSString lj_filterSpecialString:phoneNumber];
    NSString *userful = [NSString m:phoneNumber];
