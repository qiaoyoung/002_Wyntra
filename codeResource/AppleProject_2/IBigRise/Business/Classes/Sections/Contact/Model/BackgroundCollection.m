...
- (instancetype)initWithTouch:(id)first info:(id)second {
    //: self = [super init];
    self = [super init];
    //: if(self) {
    if(self) {
...
@implementation BackgroundCollection
...
- (void)setMembers:(NSArray *)members
{
    ...
    for (id<TingProtocol>member in members) {
        ...
        if ([[member cleanAcross] isEqualToString:me]) {
            //: continue;
            continue;
        }
        //: NSString *groupTitle = [member groupTitle];
        NSString *groupTitle = [member play];
    ...
    [tmp enumerateKeysAndObjectsUsingBlock:^(NSString *groupTitle, NSMutableArray *groupedMembers, BOOL *stop) {
        ...
        if (groupTitle.length) {
            ...
            [_groups addObject:[[Pair alloc] initWithTouch:groupTitle info:groupedMembers]];
        }
    //: }];
    }];
    //: [self sort];
...
- (void)buttonMagnitude:(id<TingProtocol>)member
{
    ...
    if(!pair) {
        ...
        pair = [[Pair alloc] initWithTouch:groupTitle info:members];
    }
...
- (void)breakup:(NSString *)title clear_strong:(NSArray *)members {
    ...
    Pair *pair = [[Pair alloc] initWithTouch:title info:members];
...
- (id<TingProtocol>)clansmanFromPhaseOfTheMoon:(NSString *)uid{
    ...
    for (Pair *pair in _groups) {
        ...
        for (id<TingProtocol> member in members) {
            ...
            if ([[member cleanAcross] isEqualToString:uid]) {
                //: return member;
                return member;
            }
        }
...
- (void)theGreatUnwashed
{
    ...
    [_groups enumerateObjectsUsingBlock:^(Pair *obj, NSUInteger idx, BOOL *stop) {
        ...
        [groupedMembers sortUsingComparator:^NSComparisonResult(id<TingProtocol> member1, id<TingProtocol> member2) {
            ...
            return _groupMemberComparator([member1 path], [member2 path]);
        //: }];
        }];
    //: }];
    }];
