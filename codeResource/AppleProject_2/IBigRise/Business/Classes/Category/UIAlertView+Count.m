...
static char kConst_marginSubValue;

//: @implementation UIAlertView (NTESBlock)
@implementation UIAlertView (Count)
//: - (void)showAlertWithCompletionHandler: (void (^)(NSInteger))block
- (void)resolveRange: (void (^)(NSInteger))block
{
    //: self.delegate = self;
    self.delegate = self;
    //: objc_setAssociatedObject(self,&kUIAlertViewBlockAddress,block,OBJC_ASSOCIATION_COPY);
    objc_setAssociatedObject(self,&kConst_marginSubValue,block,OBJC_ASSOCIATION_COPY);
    //: [self show];
    [self show];
}
...
- (void)length_strong:(UIAlertView *)alertView columnIndex:(NSInteger)buttonIndex
{
    //: AlertBlock block = objc_getAssociatedObject(self, &kUIAlertViewBlockAddress);
    AlertBlock block = objc_getAssociatedObject(self, &kConst_marginSubValue);
    //: if (block)
    if (block)
    {
        //: block(buttonIndex);
        ...
        objc_setAssociatedObject(self, &kConst_marginSubValue, nil, OBJC_ASSOCIATION_COPY);
    }
}
...
@implementation UIAlertController (Count)
