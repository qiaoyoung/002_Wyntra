...
+(NSString*)weekdayStr:(NSInteger)dayOfWeek
{
    ...
    daysOfWeekDict = @{@(1):LangKey(@"Sunday"),//@"星期日".ntes_localized,
                       @(2):LangKey(@"Monday"),//@"星期一".ntes_localized,
                       @(3):LangKey(@"Tuesday"),//@"星期二".ntes_localized,
                       @(4):LangKey(@"Wednesday"),//@"星期三".ntes_localized,
                       @(5):LangKey(@"Thursday"),//@"星期四".ntes_localized,
                       @(6):LangKey(@"Friday"),//@"星期五".ntes_localized,
                       @(7):LangKey(@"Saturday")};//@"星期六".ntes_localized,};
...
+(NSString*)showTime:(NSTimeInterval) msglastTime showDetail:(BOOL)showDetail
{
    ...
    {
        result = showDetail? [[NSString alloc] initWithFormat:@"%@%@ %zd:%02d",LangKey(@"before_yesterday"), result,hour,(int)msgDateComponents.minute] : LangKey(@"before_yesterday");//@"前天".ntes_localized;
    }
...
+ (NSString *)getPeriodOfTime:(NSInteger)time withMinute:(NSInteger)minute
{
    ...
    {
        showPeriodOfTime = LangKey(@"wee_hours");//@"凌晨".ntes_localized;
    }
    else if (totalMin > 5 * 60 && totalMin < 12 * 60)
    {
        showPeriodOfTime = LangKey(@"day_am");//@"上午".ntes_localized;
    }
    else if (totalMin >= 12 * 60 && totalMin <= 18 * 60)
    {
        showPeriodOfTime = LangKey(@"day_pm");//@"下午".ntes_localized;
    }
    else if ((totalMin > 18 * 60 && totalMin <= (23 * 60 + 59)) || totalMin == 0)
    {
        showPeriodOfTime = LangKey(@"day_night");//@"晚上".ntes_localized;
    }
...
+ (NSString *)tipOnMessageRevokedLocal:(NSString *)postscript {
    NSString *tip = LangKey(@"message_helper_you");//@"你".ntes_localized;
...
+ (NSString *)tipOnMessageRevoked:(NIMRevokeMessageNotification *)notification
{
    ...
    do {
        if (!notification || ![notification isKindOfClass:[NIMRevokeMessageNotification class]]) {
            tip = LangKey(@"message_helper_you");//@"你".ntes_localized;
            break;
        }
...
+ (NSString *)tipTitleFromMessageRevokeNotificationP2P:(NIMRevokeMessageNotification *)notification {
    ...
    return fromMe ? LangKey(@"message_helper_you"): LangKey(@"message_opposite");//@"对方".ntes_localized;
}
...
+ (NSString *)tipTitleFromMessageRevokeNotificationTeam:(NIMRevokeMessageNotification *)notification {
    ...
    do {
        ...
        if (revokeBySender && fromMe) {
            tipTitle = LangKey(@"message_helper_you");//@"你".ntes_localized;
            break;
        }
        ...
        if (member.type == NIMTeamMemberTypeOwner) {
            tipTitle = [LangKey(@"group_member_info_activity_team_creator") stringByAppendingString:info.showName];//@"群主".ntes_localized
        }
        else if (member.type == NIMTeamMemberTypeManager) {
            tipTitle = [LangKey(@"group_member_info_activity_team_admin") stringByAppendingString:info.showName];//@"管理员".ntes_localized
        }
    } while (false);
...
{
    NSString *clientName = [self resolveOnlineClientName:client];
    NSString *state = [NSString stringWithFormat:@"%@ %@",clientName,LangKey(@"message_online")];//@"在线".ntes_localized
    ...
    if (dict) {
        ...
        switch (onlineState) {
            case NTESOnlineStateNormal:
            {
                ...
                {
                    //桌面端不显示网络状态，只显示端
                    return [NSString stringWithFormat:@"%@ %@",clientName,LangKey(@"message_online")];//@"在线".ntes_localized
                }
                else
                {
                    ...
                    if (detail)
                    {
                        return [NSString stringWithFormat:@"%@ - %@ %@",clientName,netState,LangKey(@"message_online")];//@"在线".ntes_localized
                    }
                    else
                    {
                        return [NSString stringWithFormat:@"%@ %@",netState,LangKey(@"message_online")];//@"在线".ntes_localized
                    }
                }
            }
            case NTESOnlineStateBusy:
                return LangKey(@"online_state_event_manager_on_line_busy");//@"忙碌".ntes_localized;
            case NTESOnlineStateLeave:
                return LangKey(@"online_state_event_manager_off_line");//@"离开".ntes_localized;
            default:
                break;
        }
    }
...
{
    NSString *message = [NSString stringWithFormat:@"%@ %@",LangKey(@"login_failure"), error];//,@"自动登录失败".ntes_localized
    ...
    if ([domain isEqualToString:NIMLocalErrorDomain])
    {
        ...
        {
            message = LangKey(@"please_try_again");//@"自动登录错误次数超限，请检查网络后重试".ntes_localized;
        }
    }
    else if([domain isEqualToString:NIMRemoteErrorDomain])
    {
        ...
        {
            message = LangKey(@"wrong_password");//@"密码错误".ntes_localized;
        }
        else if(code == NIMRemoteErrorCodeExist)
        {
            message = LangKey(@"logged_another_device");//@"当前已经其他设备登录，请使用手动模式登录".ntes_localized;
        }
    }
    return message;
}
